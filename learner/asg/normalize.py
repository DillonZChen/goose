...
class AxiomConditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules):
        ...
        eff_rule_head = pddl_parser.pddl.Atom(axiom.name, [par.name for par in params])
...
class GoalConditionProxy(ConditionProxy):
    ...
    def delete_owner(self, task):
        ...
        assert False, "Disjunctive goals not (yet) implemented."
    def build_rules(self, rules):
        rule_head = pddl_parser.pddl.Atom("@goal-reachable", [])
        rule_body = condition_to_rule_body([], self.condition)
        rules.append((rule_body, rule_head))
    def get_type_map(self):
        # HACK!
...
def get_action_predicate(action):
    ...
    if isinstance(action.precondition, pddl_parser.pddl.ExistentialCondition):
        variables += [par.name for par in action.precondition.parameters]
    return pddl_parser.pddl.Atom(name, variables)

def get_axiom_predicate(axiom):
    name = axiom
    variables = [par.name for par in axiom.parameters]
    if isinstance(axiom.condition, pddl_parser.pddl.ExistentialCondition):
...
def remove_universal_quantifiers(task):
    def recurse(condition):
        ...
        if isinstance(condition, pddl_parser.pddl.UniversalCondition):
            axiom_condition = condition.negate()
            parameters = sorted(axiom_condition.free_variables())
            typed_parameters = tuple(pddl_parser.pddl.TypedObject(v, type_map[v]) for v in parameters)
            ...
            return pddl_parser.pddl.NegatedAtom(axiom.name, parameters)
        else:
...
def build_DNF(task):
    def recurse(condition):
        ...
        for part in condition.parts:
            part = recurse(part)
            if isinstance(part, pddl_parser.pddl.Disjunction):
        ...
        if isinstance(condition, pddl_parser.pddl.Disjunction):
            result_parts = other_parts
            for part in disjunctive_parts:
                result_parts.extend(part.parts)
            return pddl_parser.pddl.Disjunction(result_parts)

        # Rule (2): Distributivity disjunction/existential quantification.
        if isinstance(condition, pddl_parser.pddl.ExistentialCondition):
            parameters = condition.parameters
            result_parts = [pddl_parser.pddl.ExistentialCondition(parameters, (part,))
                            for part in disjunctive_parts[0].parts]
            return pddl_parser.pddl.Disjunction(result_parts)

        # Rule (3): Distributivity disjunction/conjunction.
        assert isinstance(condition, pddl_parser.pddl.Conjunction)
        result_parts = [pddl_parser.pddl.Conjunction(other_parts)]
        while disjunctive_parts:
            ...
            for part1 in previous_result_parts:
                for part2 in parts_to_distribute:
                    result_parts.append(pddl_parser.pddl.Conjunction((part1, part2)))
        return pddl_parser.pddl.Disjunction(result_parts)
...
def split_disjunctions(task):
    for proxy in tuple(all_conditions(task)):
        ...
        if isinstance(proxy.condition, pddl_parser.pddl.Disjunction):
...
def move_existential_quantifiers(task):
    def recurse(condition):
        ...
        for part in condition.parts:
            part = recurse(part)
            if isinstance(part, pddl_parser.pddl.ExistentialCondition):
        ...
        if isinstance(condition, pddl_parser.pddl.ExistentialCondition):
            ...
            return pddl_parser.pddl.ExistentialCondition(new_parameters, new_parts)
        ...
        new_conjunction = pddl_parser.pddl.Conjunction(new_conjunction_parts)
        return pddl_parser.pddl.ExistentialCondition(new_parameters, (new_conjunction,))
...
def eliminate_existential_quantifiers_from_axioms(task):
    ...
    for axiom in task.axioms:
        ...
        if isinstance(precond, pddl_parser.pddl.ExistentialCondition):
...
def substitute_complicated_goal(task):
    goal = task.goal
    if isinstance(goal, pddl_parser.pddl.Literal):
        return
    elif isinstance(goal, pddl_parser.pddl.Conjunction):
        for item in goal.parts:
            if not isinstance(item, pddl_parser.pddl.Literal):
                break
        else:
            return
    new_axiom = task.add_axiom([], goal)
    task.goal = pddl_parser.pddl.Atom(new_axiom.name, new_axiom.parameters)
...
def condition_to_rule_body(parameters, condition):
    ...
    if not isinstance(condition, pddl_parser.pddl.Truth):
        if isinstance(condition, pddl_parser.pddl.ExistentialCondition):
            for par in condition.parameters:
                result.append(par.get_atom())
            condition = condition.parts[0]
        if isinstance(condition, pddl_parser.pddl.Conjunction):
            parts = condition.parts
        else:
            parts = (condition,)
        for part in parts:
            if isinstance(part, pddl_parser.pddl.Falsity):
                # Use an atom in the body that is always false because
                # it is not initially true and doesn't occur in the
                # head of any rule.
                return [pddl_parser.pddl.Atom("@always-false", [])]
            assert isinstance(part, pddl_parser.pddl.Literal), "Condition not normalized: %r" % part
            if not part.negated:
                result.append(part)
    return result
