...
def variables_to_numbers(effect, conditions):
    ...
    for i, arg in enumerate(effect.args):
        if arg[0] == "?":
            rename_map[arg] = i
            new_effect_args[i] = i
    new_effect = pddl_parser.pddl.Atom(effect.predicate, new_effect_args)
    ...
    for cond in conditions:
        ...
        new_conditions.append(pddl_parser.pddl.Atom(cond.predicate, new_cond_args))
...
class Queue:
    ...
    def push(self, predicate, args):
        ...
        if eff_tuple not in self.enqueued:
            ...
            self.queue.append(pddl_parser.pddl.Atom(predicate, list(args)))
    def pop(self):
...
def compute_model(prog):
    with asg.timers.timing("Preparing model"):
    ...
    with asg.timers.timing("Computing model"):
        relevant_atoms = 0
        auxiliary_atoms = 0
        while queue:
            next_atom = queue.pop()
