...
import asg.build_model
import asg.pddl_to_prolog
...
def instantiate(task, model):
    ...
    for atom in model:
        if isinstance(atom.predicate, pddl_parser.pddl.Action):
        ...
        elif isinstance(atom.predicate, pddl_parser.pddl.Axiom):
...
def explore(task):
    prog = asg.pddl_to_prolog.translate(task)
    model = asg.build_model.compute_model(prog)
    with asg.timers.timing("Completing instantiation"):
