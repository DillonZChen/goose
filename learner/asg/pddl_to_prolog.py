...
class PrologProgram:
    ...
    def split_rules(self):
        ...
        new_rules = []
        for rule in self.rules:
            new_rules += asg.split_rules.split_rule(rule, self.new_name)
        ...
        for rule in self.rules:
            ...
            if not eff_vars.issubset(cond_vars):
                ...
                for var in sorted(eff_vars):
                    rule.add_condition(pddl_parser.pddl.Atom("@object", [var]))
        if must_add_predicate:
            print("Unbound effect variables: Adding @object predicate.")
            self.facts += [Fact(pddl_parser.pddl.Atom("@object", [obj])) for obj in self.objects]
    ...
    def convert_trivial_rules(self):
        ...
        for i, rule in enumerate(self.rules):
            if not rule.conditions:
                ...
                self.add_fact(pddl_parser.pddl.Atom(rule.effect.predicate, rule.effect.args))
        ...
        for i, var_name in enumerate(atom.args):
            if var_name[0] == "?":
                if var_name in used_variables:
                    ...
                    new_conditions.append(pddl_parser.pddl.Atom("=", [var_name, new_var_name]))
                else:
...
def translate_typed_object(prog, obj, type_dict):
    ...
    for type_name in [obj.type_name] + supertypes:
        prog.add_fact(pddl_parser.pddl.TypedObject(obj.name, type_name).get_atom())
...
def translate_facts(prog, task):
    ...
    for fact in task.init:
        assert isinstance(fact, pddl_parser.pddl.Atom) or isinstance(fact, pddl_parser.pddl.Assign)
        if isinstance(fact, pddl_parser.pddl.Atom):
            prog.add_fact(fact)

def translate(task):
    # Note: The function requires that the task has been normalized.
    with asg.timers.timing("Generating Datalog program"):
        prog = PrologProgram()
        translate_facts(prog, task)
        for conditions, effect in asg.normalize.build_exploration_rules(task):
            prog.add_rule(Rule(conditions, effect))
    with asg.timers.timing("Normalizing Datalog program", block=True):
