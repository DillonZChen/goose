...
def create_parser():
    ...
    parser.add_argument(
        "--drop", type=float, default=0.0, help="probability of an element to be zeroed"
    )
    parser.add_argument(
        "--vn", action="store_true", help="use virtual nodes (doubles runtime)"
    )

    # optimisation params
    parser.add_argument(
        "--loss", type=str, choices=["mse", "wmse", "pemse"], default="mse"
    )
    ...
    # data arguments
    parser.add_argument(
        "-r", "--rep", type=str, required=True, choices=representation.REPRESENTATIONS
    )
    parser.add_argument(
        "-n",
        "--max-nodes",
...
if __name__ == "__main__":
    ...
    try:
        ...
        for e in pbar:
            ...
            val_stats = evaluate(
                model, device, val_loader, criterion, fast_train=fast_train
            )
