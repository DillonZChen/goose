...
from learner.representation import (REPRESENTATIONS, CGraph, Representation,
                                    State)
from learner.util.stats import get_stats

from .base_wl import NO_EDGE, Histogram, WlAlgorithm
...
class Model:
    ...
    def get_weights(self) -> np.array:
        if self.model_name == "gpr":
            ...
            weights = np.sum(
                model.alpha_ @ model.X_train_ for model in self._models.values()
            )
        else:
    ...
    def get_features_sorted_by_weight(self, verbose=False):
        model_hash = self.get_hash()
        assert len(self._models) == 1, "Only works for single model"
        weights = self.get_weights()
        ret = []
        for k, v in model_hash.items():
            if verbose:
                ret.append((k, weights[v]))
            else:
                ret.append((v, weights[v]))
        ret = sorted(ret, key=lambda x: abs(x[1]), reverse=True)
    ...
    def combine_with_other_weights(self, weights: List[List[float]]):
        if not (
            hasattr(self, "_other_linear_models")
            and self._other_linear_models is not None
        ):
            self._other_linear_models = []
        for weight in weights:
            assert len(weight) == len(self.get_weights())
            self._other_linear_models.append((np.array(weight), 0))
