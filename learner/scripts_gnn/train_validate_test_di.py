...
if __name__ == "__main__":
    ...
    for repeat in range(REPEATS):
        ...
        for val_repeat in range(VAL_REPEATS):
            ...
            model_file = (
                f"di_{rep}_L{L}_H{H}_{aggr}_p{patience}_v{val_repeat}_r{repeat}"
            )
            ...
            if not os.path.exists(f"trained_models/{model_file}.dt"):
                ...
                os.system(
                    f"echo training domain independent {rep}, see {train_log_file}"
                )
                os.system(f"{cmd} > {train_log_file}")
            else:
                os.system(
                    f"echo already trained domain independent {rep}, see {train_log_file}"
                )
    ...
    for domain in GOOSE_DOMAINS:
        ...
        for repeat in range(REPEATS):
            ...
            best_model_file = (
                f"di_{rep}_{domain}_L{L}_H{H}_{aggr}_p{patience}_r{repeat}"
            )
