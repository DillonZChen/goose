...
def search_cmd(
    ...
    cmd, aux_file = search_engine(
        df, pf, model_type, m, search, seed, profile, timeout, aux_file, plan_file
    )
...
def pwl_cmd(
    ...
    cmd = (
        f"./../powerlifted/powerlifted.py --gpu "
        f"-d {df} "
        f"-i {pf} "
        f"-m {m} "
        f"-e {model_type} "
...
def fd_cmd(
    ...
    if model_type == "linear-regression-opt":
        ...
        cmd = (
            f"./../downward/fast-downward.py --search-time-limit {timeout} --sas-file {aux_file} --plan-file {plan_file} "
        ...
        if profile:
            ...
            print(
                "Running the original command to get individual commands for profiling..."
            )
            ...
            for line in output:
                if "INFO     translator command line string:" in line:
                    translator_cmd = line.replace(
                        "INFO     translator command line string:", ""
                    ).replace("\n", "")
                    continue
                if "INFO     search command line string:" in line:
                    search_cmd = line.replace(
                        "INFO     search command line string:", ""
                    )
                    continue
