...
class SDG_FEATURES(Enum):
  ACTION=0
  POSITIVE_GOAL=1
  NEGATIVE_GOAL=2
  STATE=3

class SDG_EDGE_TYPES(Enum):
  PRE_EDGE=0
  ADD_EDGE=1
  DEL_EDGE=2


""" Extended by EdgeLabelledGroundedDescriptionGraph """
class EdgeLabelledStripsProblemDescriptionGraph(Representation, ABC):
  def __init__(self, domain_pddl: str, problem_pddl: str, rep_name: str="sdg-el", node_dim: int=len(SDG_FEATURES)):
  ...
  def _compute_graph_representation(self) -> None:
    ...
    for proposition in propositions:
      ...
      if proposition in positive_goals:
        x_p = self._one_hot_node(SDG_FEATURES.POSITIVE_GOAL.value)
      elif proposition in negative_goals:
        x_p = self._one_hot_node(SDG_FEATURES.NEGATIVE_GOAL.value)
      else:
        x_p = self._zero_node()
    ...
    for action in actions:
      node_a = action.name
      x_a = self._one_hot_node(SDG_FEATURES.ACTION.value)
    ...
    for action in actions:
      ...
      for proposition in action.precondition:
        ...
        G.add_edge(u_of_edge=p_node, v_of_edge=a_node, edge_type=SDG_EDGE_TYPES.PRE_EDGE.value)
      for _, proposition in action.add_effects:  # ignoring conditional effects
        p_node = self._proposition_to_str(proposition)
        assert p_node in G.nodes, f"{p_node} not in nodes"
        assert a_node in G.nodes, f"{a_node} not in nodes"
        G.add_edge(u_of_edge=p_node, v_of_edge=a_node, edge_type=SDG_EDGE_TYPES.ADD_EDGE.value)
    ...
    for i, node in enumerate(G.nodes):
      if G.nodes[node]['x'][SDG_FEATURES.ACTION.value] == 1:
        continue
  ...
  def get_state_enc(self, state: State) -> Tuple[Tensor, Tensor]:
    ...
    for p in state:
      if p in self._node_to_i:
        x[self._node_to_i[p]][SDG_FEATURES.STATE.value] = 1
