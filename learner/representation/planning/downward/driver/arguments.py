...
def _set_components_automatically(parser, args):
    ...
    if len(args.filenames) == 1 and _looks_like_search_input(
        args.filenames[0]
    ):
...
def _set_components_and_inputs(parser, args):
    ...
    if first == "translate":
        if (
            "--help" in args.translate_options
            or "-h" in args.translate_options
        ):
            args.translate_inputs = []
        elif num_files == 1:
...
def parse_args():
    ...
    help_options = parser.add_argument_group(
        title=(
            "driver options that show information and exit "
            "(don't run planner)"
        )
    )
    ...
    if not args.version and not args.show_aliases and not args.cleanup:
        ...
        if (
            "translate" not in args.components
            or "search" not in args.components
        ):
            args.keep_sas_file = True
