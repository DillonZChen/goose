...
def translate_strips_conditions_aux(
    ...
    for fact in conditions:
        ...
        for var, val in dictionary[fact]:
            if condition.get(var) is not None and val not in condition.get(
                var
            ):
    ...
    for fact in conditions:
        if fact.negated:
            ...
            if not done and len(new_condition) != 0:
                ...
                # smallest one.
                candidates = sorted(
                    new_condition.items(), key=number_of_values
                )
...
def negate_and_translate_condition(
    condition, dictionary, ranges, mutex_dict, mutex_ranges
):
    ...
    for combination in product(*condition):
        ...
        cond = translate_strips_conditions(
            cond, dictionary, ranges, mutex_dict, mutex_ranges
        )
        if cond is not None:
...
def build_sas_operator(
    name, condition, effects_by_variable, cost, ranges, implied_facts
):
    ...
    for var, effects_on_var in effects_by_variable.items():
        ...
        for post, eff_conditions in effects_on_var.items():
            ...
            if ranges[var] == 2:
                ...
                if prune_stupid_effect_conditions(
                    var, post, eff_condition_lists, effects_on_var
                ):
...
def translate_strips_axiom(
    axiom, dictionary, ranges, mutex_dict, mutex_ranges
):
    conditions = translate_strips_conditions(
...
def translate_task(
    ...
    with timers.timing("Processing axioms", block=True):
        axioms, axiom_layer_dict = axiom_rules.handle_axioms(
            actions, axioms, goals, options.layer_strategy
        )
    ...
    return sas_tasks.SASTask(
        variables, mutexes, init, goal, operators, axioms, metric
    )
...
def pddl_to_sas(task):
    ...
    if options.add_implied_preconditions:
        with timers.timing("Building implied facts dictionary..."):
            implied_facts = build_implied_facts(
                strips_to_sas, groups, mutex_groups
            )
    else:
        implied_facts = {}
    ...
    print(
        "%d effect conditions simplified" % simplified_effect_condition_counter
    )
    print(
        "%d implied preconditions added" % added_implied_precondition_counter
    )
...
def dump_statistics(sas_task):
    ...
    print(
        ...
        % len(
            [layer for layer in sas_task.variables.axiom_layers if layer >= 0]
        )
    )
