...
class SASTask:
    ...
    def __init__(
        ...
        self.axioms = sorted(
            axioms, key=lambda axiom: (axiom.condition, axiom.effect)
        )
        self.metric = metric
        if DEBUG:
            self.validate()
...
class SASVariables:
    ...
    def validate(self):
        ...
        """
        assert (
            len(self.ranges) == len(self.axiom_layers) == len(self.value_names)
        )
...
class SASOperator:
    def __init__(
        ...
        pre_post: List[Tuple[int, int, int, List[VarValPair]]],
        cost: int,
    ) -> None:
        self.name = name
...
class SASAxiom:
    def __init__(
        self, condition: List[VarValPair], effect: VarValPair
    ) -> None:
