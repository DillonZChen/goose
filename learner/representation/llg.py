    ...
    def _compute_graph_representation(self) -> None:
        ...
        for pred in self.problem.predicates:
            for obj in self.problem.objects:
                G.add_edge(
                    u_of_edge=pred.name, v_of_edge=obj.name, edge_label=LLG_EDGE_LABELS["neutral"]
                )
        ...
        for fact in goals:
            ...
            for i, arg in enumerate(args):
                ...
                G.add_edge(
                    u_of_edge=goal_var_node, v_of_edge=arg, edge_label=LLG_EDGE_LABELS["ground"]
                )
        ...
        for action in self.problem.actions:
            ...
            for i, arg in enumerate(action.parameters):
                ...
                G.add_edge(
                    u_of_edge=action.name, v_of_edge=arg_node, edge_label=LLG_EDGE_LABELS["neutral"]
                )
            ...
            def deal_with_action_prec_or_eff(predicates, edge_label):
                for z, predicate in enumerate(predicates):
                    ...
                    assert pred in G.nodes()
                    G.add_edge(
                        u_of_edge=pred, v_of_edge=aux_node, edge_label=LLG_EDGE_LABELS[edge_label]
                    )
    ...
    def state_to_cgraph(self, state: List[Tuple[str, List[str]]]) -> CGraph:
        ...
        for fact in state:
            ...
            for k, arg in enumerate(args):
                ...
                # connect variable to predicate
                c_graph.add_edge(
                    u_of_edge=node, v_of_edge=arg_node, edge_label=LLG_EDGE_LABELS["ground"]
                )
                c_graph.add_edge(
                    v_of_edge=node, u_of_edge=arg_node, edge_label=LLG_EDGE_LABELS["ground"]
                )

                # connect variable to object
                c_graph.add_edge(
                    u_of_edge=arg_node, v_of_edge=arg, edge_label=LLG_EDGE_LABELS["ground"]
                )
                c_graph.add_edge(
                    v_of_edge=arg_node, u_of_edge=arg, edge_label=LLG_EDGE_LABELS["ground"]
                )
