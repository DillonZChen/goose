...
from . import aliases, returncodes, util
...
EXAMPLES = [
    (
        ...
        ["misc/tests/benchmarks/gripper/prob01.pddl", "--search", '"astar(lmcut())"'],
    ),
    ("Translate and run no search:", ["--translate", "misc/tests/benchmarks/gripper/prob01.pddl"]),
    ("Run predefined configuration (LAMA-2011) on translated task:", ["--alias", "seq-sat-lama-2011", "output.sas"]),
    (
        "Run a portfolio on a translated task:",
        ["--portfolio", str(EXAMPLE_PORTFOLIO), "--search-time-limit", "30m", "output.sas"],
    ),
    (
    ...
    (
        ...
        ["--validate", "misc/tests/benchmarks/gripper/prob01.pddl", "--search", '"astar(cegar())"'],
    ),
    (
        ...
        [
            "misc/tests/benchmarks/gripper/prob01.pddl",
            "--search",
            '"let(hff, ff(), eager_greedy([hff], preferred=[hff]))"',
        ],
    ),
    (
    ...
    for pos, (name1, is_specified1) in enumerate(args):
        for name2, is_specified2 in args[pos + 1 :]:
            if is_specified1 and is_specified2:
                print_usage_and_exit_with_driver_input_error(parser, "cannot combine %s with %s" % (name1, name2))
    if required and not any(is_specified for _, is_specified in args):
        print_usage_and_exit_with_driver_input_error(
            parser, "exactly one of {%s} has to be specified" % ", ".join(name for name, _ in args)
        )
...
def _get_pddl_input_files(args, parser, component):
    ...
    if num_files == 1:
        ...
        print_usage_and_exit_with_driver_input_error(parser, f"{component} needs one or two PDDL input files")
    return [domain, task]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=DESCRIPTION, epilog=EPILOG, formatter_class=RawHelpFormatter, add_help=False
    )

    help_options = parser.add_argument_group(
        title=("driver options that show information and exit " "(don't run planner)")
    )
    ...
    help_options.add_argument("-v", "--version", action="store_true", help="print version number and exit")
    help_options.add_argument(
        "--show-aliases", action="store_true", help="show the known aliases (see --alias) and exit"
    )
    ...
    components.add_argument("--run-all", action="store_true", help="run all components of the planner")
    components.add_argument("--translate", action="store_true", help="run translator component")
    components.add_argument("--search", action="store_true", help="run search component")

    limits = parser.add_argument_group(title="time and memory limits", description=LIMITS_HELP)
    for component in COMPONENTS_PLUS_OVERALL:
        limits.add_argument("--{}-time-limit".format(component))
        limits.add_argument("--{}-memory-limit".format(component))

    driver_other = parser.add_argument_group(title="other driver options")
    driver_other.add_argument("--alias", help="run a config with an alias (e.g. seq-sat-lama-2011)")
    ...
    driver_other.add_argument("--debug", action="store_true", help="alias for --build=debug --validate")
    driver_other.add_argument(
        "--validate", action="store_true", help='validate plans (implied by --debug); needs "validate" (VAL) on PATH'
    )
    driver_other.add_argument(
        "--log-level",
    ...
    driver_other.add_argument("--portfolio", metavar="FILE", type=Path, help="run a portfolio specified in FILE")
    driver_other.add_argument(
        "--portfolio-bound",
        metavar="VALUE",
        default=None,
    ...
    driver_other.add_argument(
        "--portfolio-single-plan", action="store_true", help="abort satisficing portfolio after finding the first plan"
    )

    driver_other.add_argument(
        "--cleanup", action="store_true", help="clean up temporary files (translator output and plan files) and exit"
    )

    parser.add_argument(
        "planner_args", nargs=argparse.REMAINDER, help="file names and options passed on to planner components"
    )
    ...
    if args.build and args.debug:
        print_usage_and_exit_with_driver_input_error(
            parser,
            "The option --debug is an alias for --build=debug " "--validate. Do no specify both --debug and --build.",
        )
    if not args.build:
        if args.debug:
            args.build = "debug"
    ...
    if args.alias:
        try:
            ...
            print_usage_and_exit_with_driver_input_error(parser, "unknown alias: %r" % args.alias)

    if args.portfolio_bound is not None and not args.portfolio:
        print_usage_and_exit_with_driver_input_error(parser, "--portfolio-bound may only be used for portfolios.")
    if args.portfolio_bound is not None and args.portfolio_bound < 0:
        print_usage_and_exit_with_driver_input_error(parser, "--portfolio-bound must not be negative.")
