...
from . import limits, returncodes
from .aliases import ALIASES, PORTFOLIOS
from .arguments import EXAMPLES
from .call import _replace_paths_with_strings, check_call
from .run_components import REL_SEARCH_PATH, get_executable
from .util import REPO_ROOT_DIR, find_domain_path


def cleanup():
    subprocess.check_call([sys.executable, "fast-downward.py", "--cleanup"], cwd=REPO_ROOT_DIR)
...
def test_portfolios():
    for name, portfolio in PORTFOLIOS.items():
        parameters = ["--portfolio", portfolio, "--search-time-limit", "30m", "output.sas"]
        run_driver(parameters)
...
def _get_portfolio_configs(portfolio: Path):
    ...
    try:
        ...
        traceback.print_exc()
        raise SyntaxError(f"The portfolio {portfolio} could not be loaded.")
...
def _run_search(config):
    check_call("search", [get_executable("release", REL_SEARCH_PATH)] + list(config), stdin="output.sas")
