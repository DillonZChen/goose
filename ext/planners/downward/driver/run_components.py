...
_VALIDATE_NAME = shutil.which(f"validate{BINARY_EXT}") or shutil.which(f"Validate{BINARY_EXT}")
    ...
    if not build_dir.exists():
        ...
        if not build_dir.exists():
            returncodes.exit_with_driver_input_error(
                f"Could not find build '{build}' at {build_dir}. " f"Please run './build.py {build}'."
            )

    abs_path = build_dir / rel_path
    if not abs_path.exists():
        returncodes.exit_with_driver_input_error(
            f"Could not find '{rel_path}' in build '{build}'. " f"Please run './build.py {build}'."
        )
...
def run_translate(args):
    ...
    stderr, returncode = call.get_error_output_and_returncode(
        "translator", cmd, time_limit=time_limit, memory_limit=memory_limit
    )
...
def run_search(args):
    ...
    plan_manager = PlanManager(
        args.plan_file, portfolio_bound=args.portfolio_bound, single_plan=args.portfolio_single_plan
    )
    ...
    if args.portfolio:
        ...
        return portfolio_runner.run(
            args.portfolio, executable, args.search_input, plan_manager, time_limit, memory_limit
        )
    else:
        if not args.search_options:
            returncodes.exit_with_driver_input_error("search needs --alias, --portfolio, or search options")
...
def run_validate(args):
    if not VALIDATE:
        returncodes.exit_with_driver_input_error("Error: Trying to run validate but it was not found on the PATH.")
