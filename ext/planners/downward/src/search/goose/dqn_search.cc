...
namespace dqn_search {
DualQueueEagerSearch::DualQueueEagerSearch(
...
void DualQueueEagerSearch::initialize() {
...
void DualQueueEagerSearch::print_statistics() const {
    statistics.print_detailed_statistics();
    search_space.print_statistics();
    pruning_method->print_statistics();
}

SearchStatus DualQueueEagerSearch::step() {
...
void DualQueueEagerSearch::reward_progress() {
    // Boost the "preferred operator" open lists somewhat whenever
    // one of the heuristics finds a state with a new best h value.
    open_list->boost_preferred();
}

void DualQueueEagerSearch::dump_search_space() const {
    search_space.dump(task_proxy);
}

void DualQueueEagerSearch::start_f_value_statistics(EvaluationContext &eval_context) {
    if (f_evaluator) {
...
void DualQueueEagerSearch::update_f_value_statistics(EvaluationContext &eval_context) {
    if (f_evaluator) {
        int f_value = eval_context.get_evaluator_value(f_evaluator.get());
        statistics.report_f_value_progress(f_value);
    }
...
void add_dqn_search_options_to_feature(
...
get_dqn_search_arguments_from_options(const plugins::Options &opts) {
    return tuple_cat(
