...
namespace plugin_dual_queue_novelty_greedy {
class DualQueueNoveltyGreedySearchFeature
    : public plugins::TypedFeature<SearchAlgorithm, dqn_search::DualQueueEagerSearch> {
public:
    DualQueueNoveltyGreedySearchFeature() : TypedFeature("dual_queue_novelty_greedy") {
        document_title("Dual queue novelty search (eager)");
        document_synopsis("");
        ...
        dqn_search::add_dqn_search_options_to_feature(
            *this, "eager_greedy");
    ...
    virtual shared_ptr<dqn_search::DualQueueEagerSearch> create_component(
        const plugins::Options &opts,
        const utils::Context &context) const override {
        plugins::verify_list_non_empty<shared_ptr<Evaluator>>(
            context, opts, "evals");

        return plugins::make_shared_from_arg_tuples<dqn_search::DualQueueEagerSearch>(
            ...
            nullptr,
            opts.get_list<shared_ptr<Evaluator>>("preferred"),
            dqn_search::get_dqn_search_arguments_from_options(opts)
            );
    }
};

static plugins::FeaturePlugin<DualQueueNoveltyGreedySearchFeature> _plugin;
}
