...
def compute_necessary_atoms(dependencies, goals, operators):
    ...
    for op in operators:
        derived_preconditions = (
            l.positive() for l in op.precondition if l.positive() in dependencies.derived_variables
        )
    ...
    while atoms_to_process:
        ...
        for body_atom in chain(dependencies.positive_dependencies[atom], dependencies.negative_dependencies[atom]):
