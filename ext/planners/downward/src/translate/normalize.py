...
class PreconditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules):
        ...
        # body.
        pne = None
        if isinstance(action.cost, pddl.Increase) and isinstance(
            action.cost.expression, pddl.PrimitiveNumericExpression
        ):
            pne = action.cost.expression

        rule_body = condition_to_rule_body(action.parameters, self.condition, pne)
...
def condition_to_rule_body(
    parameters: Sequence[pddl.TypedObject],
