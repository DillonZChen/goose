...
class Action:
    ...
    def instantiate(self, var_mapping, init_facts, init_assignments, fluent_facts, objects_by_type, metric):
        ...
        precondition = []
        try:
            self.precondition.instantiate(var_mapping, init_facts, fluent_facts, precondition)
        except conditions.Impossible:
            return None
        effects = []
        for eff in self.effects:
            eff.instantiate(var_mapping, init_facts, fluent_facts, objects_by_type, effects)
        if effects:
            if metric:
                if self.cost is None:
                    cost = 0
                else:
                    cost = int(self.cost.instantiate(var_mapping, init_assignments).expression.value)
            else:
                cost = 1
...
class PropositionalAction:
    def __init__(
        self, name: str, precondition: List[Literal], effects: List[Tuple[List[Literal], Literal]], cost: int
    ):
        self.name = name
        self.precondition = precondition
        self.add_effects = []
