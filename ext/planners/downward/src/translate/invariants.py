...
def instantiate_factored_mapping(pairs):
    ...
    # from preimg to img.
    part_mappings = [
        [list(zip(preimg, perm_img)) for perm_img in itertools.permutations(img)] for (preimg, img) in pairs
    ]
...
class InvariantPart:
    ...
    def get_parameters(self, literal):
        ...
        return dict((arg, literal.args[pos]) for pos, arg in enumerate(self.args) if pos != self.omitted_pos)

    def instantiate(self, parameters_tuple):
        args = [parameters_tuple[arg] if arg != COUNTED else "?X" for arg in self.args]
...
class Invariant:
    ...
    def _get_cover_equivalence_conjunction(self, literal):
        ...
        equalities = [(arg, literal.args[pos]) for pos, arg in enumerate(part.args) if arg != COUNTED]
    ...
    def _operator_too_heavy(self, h_action):
        add_effects = [
            ...
            if not eff.literal.negated and self.predicate_to_part.get(eff.literal.predicate)
        ]
    ...
    def _operator_unbalanced(self, action, enqueue_func):
        relevant_effs = [eff for eff in action.effects if self.predicate_to_part.get(eff.literal.predicate)]
        ...
        for eff in add_effects:
            if self._add_effect_unbalanced(action, eff, del_effects, enqueue_func):
                return True
        return False

    def _add_effect_unbalanced(self, action, add_effect, del_effects, enqueue_func):
        ...
        params = [p.name for p in itertools.chain(action.parameters, add_effect.parameters)]
        ...
        for del_effect in del_effects:
            if self._balances(del_effect, add_effect, add_effect_produced_by_pred, add_cover, param_system):
                return False
    ...
    def _balances(self, del_effect, add_effect, produced_by_pred, add_cover, param_system):
        ...
        balance_system = self._balance_system(add_effect, del_effect, produced_by_pred)
    ...
    def _balance_system(self, add_effect, del_effect, literals_by_pred):
        ...
        """
        system = constraints.ConstraintSystem()
        for literal in itertools.chain(get_literals(del_effect.condition), [del_effect.literal.negate()]):
            possibilities = []
            ...
            for match in literals_by_pred[literal.predicate]:
                if match.negated == literal.negated:
                    ...
                    ec = constraints.EqualityConjunction(list(zip(literal.args, match.args)))
