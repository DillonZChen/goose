...
class BalanceChecker:
    ...
    def add_inequality_preconds(self, action, reachable_action_params):
        ...
        if inequal_params:
            ...
            return pddl.Action(
                action.name, action.parameters, action.num_external_parameters, precond, action.effects, action.cost
            )
        else:
            return action
...
def get_initial_invariants(task):
    for predicate in get_fluents(task):
        ...
        for omitted in range(len(predicate.arguments)):
            inv_args = all_args[0:omitted] + [invariants.COUNTED] + all_args[omitted:-1]
...
def useful_groups(invariants, initial_facts):
    ...
    for atom in initial_facts:
        ...
        for invariant in predicate_to_invariants.get(atom.predicate, ()):
            ...
            parameters_tuple = tuple(parameters[var] for var in range(invariant.arity()))
    ...
    useful_groups = [group_key for group_key in nonempty_groups.keys() if group_key not in overcrowded_groups]
