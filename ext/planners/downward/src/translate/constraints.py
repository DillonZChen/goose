...
class EqualityConjunction:
    ...
    def _compute_representatives(self):
        ...
        for eq_class in self._eq_classes.values():
            ...
            variables = [item for item in eq_class if isinstance(item, int) or item.startswith("?")]
            objects = [item for item in eq_class if not isinstance(item, int) and not item.startswith("?")]
...
class ConstraintSystem:
    ...
    def __str__(self):
        ...
        for eq_DNF in self.equality_DNFs:
            disj = " or ".join([str(eq_conjunction) for eq_conjunction in eq_DNF])
            disj = "(%s)" % disj
    ...
    def _combine_equality_conjunctions(self, eq_conjunctions: Iterable[EqualityConjunction]) -> None:
        all_eq = itertools.chain.from_iterable(c.equalities for c in eq_conjunctions)
    ...
    def is_solvable(self):
        ...
        for eq_conjunction in itertools.product(*self.equality_DNFs):
            ...
            if any(
                not isinstance(representative.get(s, s), int) and representative.get(s, s)[0] != "?"
                for s in self.not_constant
            ):
                continue
            if any(not inequality_disjunction_ok(d, representative) for d in self.ineq_disjunctions):
                continue
            return True
        return False
