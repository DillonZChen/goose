...
SYNTAX_ACTION = "(:action NAME [:parameters PARAMETERS]? " "[:precondition PRECONDITION]? :effect EFFECT)"
...
class Context:
    ...
    def expected_named_block_error(self, alist, expected, *args, **kwargs):
        self.error(
            f"Expected a non-empty block starting with any of the " f"following words: {', '.join(expected)}",
            item=alist,
            *args,
            **kwargs,
        )
...
def parse_typed_list(context, alist, only_variables=False, constructor=construct_typed_object, default_type="object"):
            ...
            with context.layer(f"Parsing {group_number}. group of typed list"):
                try:
                    ...
                    if separator_position == len(alist) - 1:
                        context.error(f"Type missing after '{TYPED_LIST_SEPARATOR}'.", alist)
                    ...
                    if not (
                        ...
                        or (
                            _type
                            and _type[0] == "either"
                            and all(isinstance(_sub_type, str) for _sub_type in _type[1:])
                        )
                    ):
                        context.error("Type value is expected to be a single word " "or '(either WORD*)")
                for item in items:
        ...
        for no, entry in enumerate(alist):
            with context.layer(f"Parsing {no}. predicate"):
                if not isinstance(entry, list):
                    context.error("Invalid predicate definition.", syntax=SYNTAX_PREDICATE)
...
def parse_function(context, alist, type_name):
    with context.layer("Parsing function name"):
        if not isinstance(alist, list) or len(alist) == 0:
            context.error("Invalid definition of function.", syntax=SYNTAX_FUNCTION)
        name = alist[0]
        if not isinstance(name, str):
...
def parse_condition_aux(context, alist, negated, type_dict, predicate_dict):
    ...
    if tag in ("and", "or", "not", "imply"):
        ...
        if tag == "imply":
            if len(args) != 2:
                context.error("'imply' expects exactly two arguments.", syntax=SYNTAX_CONDITION_IMPLY)
        if tag == "not":
            if len(args) != 1:
                context.error("'not' expects exactly one argument.", syntax=SYNTAX_CONDITION_NOT)
            negated = not negated
    elif tag in ("forall", "exists"):
        if len(alist) != 3:
            context.error("'forall' and 'exists' expect exactly two arguments.", syntax=SYNTAX_CONDITION_FORALL_EXISTS)
        if not isinstance(alist[1], list) or not alist[1]:
            context.error(
                "The first argument (VARIABLES) of 'forall' and 'exists' is " "expected to be a non-empty block.",
    ...
    for nb_arg, arg in enumerate(args, start=1):
        if not isinstance(arg, list) or not arg:
            context.error(
                ...
                item=arg,
                syntax=CONDITION_TAG_TO_SYNTAX[tag],
            )

    if tag == "imply":
        parts = [
            parse_condition_aux(context, args[0], not negated, type_dict, predicate_dict),
            parse_condition_aux(context, args[1], negated, type_dict, predicate_dict),
        ]
        tag = "or"
    else:
        parts = [parse_condition_aux(context, part, negated, type_dict, predicate_dict) for part in args]
...
def parse_literal(context, alist, type_dict, predicate_dict, negated=False):
    with context.layer("Parsing literal"):
        if not alist:
            context.error(
                "Literal definition has to be a non-empty block.", alist, syntax=SYNTAX_LITERAL_POSSIBLY_NEGATED
            )
        if alist[0] == "not":
            if len(alist) != 2:
                context.error(
                    ...
                    alist,
                    syntax=SYNTAX_LITERAL_NEGATED,
                )
            alist = alist[1]
            if not isinstance(alist, list) or not alist:
                context.error(
                    "Definition of negated literal has to be a non-empty block.", alist, syntax=SYNTAX_LITERAL
                )
            negated = not negated
        ...
        pred_id, arity = _get_predicate_id_and_arity(context, predicate_name, type_dict, predicate_dict)

        if arity != len(alist) - 1:
            context.error(
                f"Predicate '{predicate_name}' of arity {arity} used" f" with {len(alist) - 1} arguments.", alist
            )
    ...
    if tag == "and":
        ...
        for eff in alist[1:]:
            if not isinstance(eff, list):
                context.error("All sub-effects of a conjunction have to be blocks.", eff)
            effects.append(parse_effect(context, eff, type_dict, predicate_dict))
        return pddl.ConjunctiveEffect(effects)
    elif tag == "forall":
        if len(alist) != 3:
            context.error("'forall' effect expects exactly two arguments.", syntax=SYNTAX_EFFECT_FORALL)
        if not isinstance(alist[1], list):
            context.expected_list_error(
                "First argument (VARIABLES) of 'forall'", alist[1], syntax=SYNTAX_EFFECT_FORALL
            )
        parameters = parse_typed_list(context, alist[1])
        if not isinstance(alist[2], list):
            context.expected_list_error("Second argument (EFFECT) of 'forall'", alist[2], syntax=SYNTAX_EFFECT_FORALL)
        effect = parse_effect(context, alist[2], type_dict, predicate_dict)
        return pddl.UniversalEffect(parameters, effect)
    elif tag == "when":
        if len(alist) != 3:
            context.error("'when' effect expects exactly two arguments.", syntax=SYNTAX_EFFECT_WHEN)
        if not isinstance(alist[1], list):
            context.error(
                "First argument (CONDITION) of 'when' is expected to be a block", alist[1], syntax=SYNTAX_EFFECT_WHEN
            )
        condition = parse_condition(context, alist[1], type_dict, predicate_dict)
        if not isinstance(alist[2], list):
            context.expected_list_error("Second argument (EFFECT) of 'when'", alist[2], syntax=SYNTAX_EFFECT_WHEN)
        effect = parse_effect(context, alist[2], type_dict, predicate_dict)
        return pddl.ConditionalEffect(condition, effect)
    elif tag == "increase":
        if len(alist) != 3 or alist[1] != ["total-cost"]:
            context.error("'increase' expects two arguments", alist, syntax=SYNTAX_EFFECT_INCREASE)
...
def parse_expression(context, exp):
    with context.layer("Parsing expression"):
        if isinstance(exp, list):
            if len(exp) < 1:
                context.error("Expression cannot be an empty block.", syntax=SYNTAX_EXPRESSION)
            ...
            context.error("Expression cannot be a negative number", syntax=SYNTAX_EXPRESSION)
        else:
...
def parse_assignment(context, alist):
    with context.layer("Parsing Assignment"):
        if len(alist) != 3:
            context.error("Assignment expects two arguments", syntax=SYNTAX_ASSIGNMENT)
        op = alist[0]
        ...
        if op == "=":
            ...
            context.error(f"Unsupported assignment operator '{op}'." f" Use '=' or 'increase'.")


def parse_action(context, alist, type_dict, predicate_dict):
    with context.layer("Parsing action name"):
        if len(alist) < 4:
            context.error("Expecting block with at least 3 arguments for an action.", syntax=SYNTAX_ACTION)
        iterator = iter(alist)
        ...
        try:
            with context.layer("Parsing parameters"):
                ...
                if parameters_tag_opt == ":parameters":
                    ...
                    if not isinstance(parameters_list, list):
                        context.expected_list_error("Parameters", parameters_list, syntax=SYNTAX_ACTION)
                    parameters = parse_typed_list(context, parameters_list, only_variables=True)
            ...
            with context.layer("Parsing precondition"):
                if precondition_tag_opt == ":precondition":
                    ...
                    if not isinstance(precondition_list, list):
                        context.expected_list_error("Precondition", precondition_list, syntax=SYNTAX_ACTION)
                    if not precondition_list:
                        # Note that :precondition () is allowed in PDDL.
                        precondition = pddl.Conjunction([])
                    else:
                        precondition = parse_condition(context, precondition_list, type_dict, predicate_dict)
            ...
            with context.layer("Parsing effect"):
                if effect_tag != ":effect":
                    context.error("Effect tag is expected to be ':effect'", effect_tag, syntax=SYNTAX_ACTION)
                ...
                eff = []
                if effect_list:
                    cost = parse_effects(context, effect_list, eff, type_dict, predicate_dict)
...
def parse_axiom(context, alist, type_dict, predicate_dict):
    with context.layer("Parsing derived predicate"):
        if len(alist) != 3:
            context.error("Expecting block with exactly three elements", syntax=SYNTAX_AXIOM)
        assert alist[0] == ":derived"
        if not isinstance(alist[1], list):
            context.expected_list_error("The first argument (PREDICATE)", syntax=SYNTAX_AXIOM)
        predicate = parse_predicate(context, alist[1])
    with context.layer(f"Parsing condition for derived predicate '{predicate}'"):
        if not isinstance(alist[2], list):
            context.error("The second argument (CONDITION) is expected to be a block.", syntax=SYNTAX_AXIOM)
        condition = parse_condition(context, alist[2], type_dict, predicate_dict)
        return pddl.Axiom(predicate.name, predicate.arguments, len(predicate.arguments), condition)
        ...
        with context.layer(f"Parsing {no}. axiom/action entry"):
            ...
            if entry[0] == ":derived":
                with context.layer(f"Parsing {len(the_axioms) + 1}. axiom"):
                    the_axioms.append(parse_axiom(context, entry, type_dict, predicate_dict))
            else:
    ...
    for no, fact in enumerate(alist[1:], start=1):
        with context.layer(f"Parsing {no}. element in init block"):
            if not isinstance(fact, list) or not fact:
                context.error("Invalid fact.", syntax=f"{SYNTAX_LITERAL_POSSIBLY_NEGATED} or {SYNTAX_ASSIGNMENT}")
            if fact[0] == "=":
                try:
                ...
                if not isinstance(assignment.expression, pddl.NumericConstant):
                    context.error("Illegal assignment in initial state specification.", assignment)
                if assignment.fluent in initial_assignments:
                    ...
                    if assignment.expression == prev.expression:
                        print(f"Warning: {assignment} is specified twice " f"in initial state specification")
                    else:
                        context.error(
                ...
                check_atom_consistency(context, atom, initial_proposition_values, False)
                initial_proposition_values[atom] = False
            else:
                atom = pddl.Atom(fact[0], fact[1:])
                check_atom_consistency(context, atom, initial_proposition_values, True)
                initial_proposition_values[atom] = True
    initial.extend(atom for atom, val in initial_proposition_values.items() if val is True)
    return initial
...
def parse_task(domain_pddl, task_pddl):
    ...
    (
        ...
        actions,
        axioms,
    ) = parse_domain_pddl(context, domain_pddl)
    ...
    task_name, task_domain_name, task_requirements, objects, init, goal, use_metric = parse_task_pddl(
        context, task_pddl, type_dict, predicate_dict
    )

    if domain_name != task_domain_name:
        context.error(
            f"The domain name specified by the task "
...
def parse_domain_pddl(context, domain_pddl):
    ...
    with context.layer("Parsing domain"):
        ...
        with context.layer("Parsing domain name"):
            ...
            if (
                not check_named_block(domain_line, ["domain"])
                or len(domain_line) != 2
                or not isinstance(domain_line[1], str)
            ):
                context.error("Invalid definition of domain name.", syntax=SYNTAX_DOMAIN_DOMAIN_NAME)
            yield domain_line[1]
        ...
        correct_order = [":requirements", ":types", ":constants", ":predicates", ":functions"]
        ...
        for opt in iterator:
            ...
            if seen_fields and correct_order.index(seen_fields[-1]) > correct_order.index(field):
            ...
            if field == ":requirements":
                ...
                with context.layer("Parsing types"):
                    the_types.extend(parse_typed_list(context, opt[1:], constructor=construct_type))
                ...
                the_predicates += [
                    pddl.Predicate("=", [pddl.TypedObject("?x", "object"), pddl.TypedObject("?y", "object")])
                ]
            elif field == ":functions":
                with context.layer("Parsing functions"):
                    the_functions = parse_typed_list(
                        context, opt[1:], constructor=parse_function, default_type="number"
                    )
        ...
        the_axioms, the_actions = parse_axioms_and_actions(context, entries, type_dict, predicate_dict)
...
def parse_task_pddl(context, task_pddl, type_dict, predicate_dict):
    ...
    with context.layer("Parsing task"):
        ...
        with context.layer("Parsing problem name"):
            ...
            if (
                not check_named_block(problem_line, ["problem"])
                or len(problem_line) != 2
                or not isinstance(problem_line[1], str)
            ):
                context.error("Invalid problem name definition", problem_line, syntax=SYNTAX_TASK_PROBLEM_NAME)
        ...
        with context.layer("Parsing domain name"):
            ...
            if (
                ...
                context.error("Invalid domain name definition", domain_line, syntax=SYNTAX_TASK_DOMAIN_NAME)
            yield domain_line[1]
        ...
        with context.layer("Parsing goal"):
            if (
                not check_named_block(goal, [":goal"])
                or len(goal) != 2
...
def check_atom_consistency(context, atom, initial_proposition_values, atom_value):
