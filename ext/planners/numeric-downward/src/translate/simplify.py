...
def build_dtgs(task):
    ...
    # NFD: numeric Variables have a domain size of -1 and will not be considered here
    dtgs = [DomainTransitionGraph(init, size) for (init, size) in zip(init_vals, sizes) if size > 0]
...
class VarValueRenaming(object):
    ...
    def apply_to_task(self, task):
        ...
        self.apply_to_axioms(task.axioms, task.comp_axioms)
    ...
    def translate_global_constraint(self, constraint):
        new_constraint = self.translate_pair(constraint)
        assert new_constraint, "new constraint = %s" % new_constraint
        print("Simplifyed global constraint to new variable ordering %s" % [new_constraint])
        if new_constraint[1] == always_false:
            assert False, "Task is unsolvable, Global constraint cannot be satisfied"
        if new_constraint[1] == always_true:
            assert (
                False
            ), "Regular Task without global constraint"  # somehow the created axiom is not simplified to always true and we never end up here
    ...
    def apply_to_axioms(self, axioms, comp_axioms):
        new_axioms = []
        num_removed = 0
        for axiom in axioms:
            try:
        ...
        for cax in comp_axioms:
            #            print("Renaming affected variable of compare axiom #%d to new index #%d" % (cax.effect, self.new_var_nos[cax.effect]))
            cax.effect = self.new_var_nos[cax.effect]
    ...
    def translate_operator(self, op):
        ...
        return sas_tasks.SASOperator(
            name=op.name, prevail=new_prevail, pre_post=new_pre_post, assign_effect=new_assign_effect, cost=op.cost
        )
    ...
    def translate_pre_post(self, pre_post_entry, conditions_dict):
        ...
        assert new_pre is not always_false, (
            "This function should only be called for operators " "whose applicability conditions are deemed possible."
        )
        ...
        assert new_pre is not always_true, (
            "if this pre_post changes the value and can fire, " "new_pre cannot be always_true"
        )

        return new_var_no, new_pre, new_post, new_cond

    def translate_assign_effect(self, assign_effect):
        return assign_effect

    #        (var_no, ass_op, ass_var, cond) = assign_effects
    #        try:
    #            self.translate_pairs_in_place(cond)
    #        except Impossible:
    #            raise DoesNothing
    #        return var_no, ass_op, ass_var, cond
...
def filter_unreachable_propositions(sas_task):
    ...
    #    print("simplify, filter_unreachable_propositons: printing dtgs")
    #    for dtg in dtgs:
    #        dtg.dump()
