...
def build_DNF(task):
    def recurse(condition):
        ...
        if isinstance(condition, pddl.ExistentialCondition):
            ...
            result_parts = [pddl.ExistentialCondition(parameters, (part,)) for part in disjunctive_parts[0].parts]
            return pddl.Disjunction(result_parts)
        ...
        if isinstance(aexp, pddl.PrimitiveNumericExpression):
            ...
            if not aexp.symbol in function_names:
                ...
                if len(aexp.args) == 0:
                    ...
                    raise Error(
                        "Don't know the parameters of function %s with arity %d" % (aexp.symbol, len(aexp.args))
                    )
                    assert False
...
def verify_axiom_predicates(task):
    ...
    for fact in task.init:
        ...
        if getattr(fact, "predicate", None) in axiom_names:
            raise SystemExit("error: derived predicate %r appears in :init fact '%s'" % (fact.predicate, fact))
...
def condition_to_rule_body(parameters, condition):
    ...
    if not isinstance(condition, pddl.Truth):
        ...
        if isinstance(condition, pddl.Conjunction):
            ...
            parts = (condition,)
        for part in parts:
            assert isinstance(part, pddl.Literal) or isinstance(part, pddl.FunctionComparison), (
                "Condition not normalized (part of class %s)" % part.__class__
            )
