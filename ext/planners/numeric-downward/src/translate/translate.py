...
def translate_strips_conditions_aux(
    conditions, dictionary, ranges, numeric_dictionary, comparison_axioms, mutex_check=False
):
    condition = {}
...
def translate_strips_conditions(
    conditions, dictionary, ranges, numeric_dictionary, mutex_dict, mutex_ranges, comp_axioms
):
    if not conditions:
    ...
    if (
        translate_strips_conditions_aux(conditions, mutex_dict, mutex_ranges, numeric_dictionary, comp_axioms, True)
        is None
    ):
        return None
...
def translate_strips_operator(
    ...
    conditions = translate_strips_conditions(
        operator.precondition, dictionary, ranges, numeric_dictionary, mutex_dict, mutex_ranges, comp_axioms
    )
...
def negate_and_translate_condition(condition, dictionary, ranges, numeric_dict, mutex_dict, mutex_ranges, comp_axioms):
    ...
    for combination in product(*condition):
        ...
        cond = translate_strips_conditions(
            cond, dictionary, ranges, numeric_dict, mutex_dict, mutex_ranges, comp_axioms
        )
        if cond is not None:
...
def translate_strips_operator_aux(
    ...
    for conditions, assignment in operator.assign_effects:
        ...
        if assignment.expression in numeric_dictionary:
            ass_effects_by_variable[numeric_dictionary[assignment.fluent]][
                (assignment.symbol, numeric_dictionary[assignment.expression])
    ...
    for var in del_effects_by_variable:
        no_add_effect_condition = negate_and_translate_condition(
            add_conds_by_variable[var], dictionary, ranges, numeric_dictionary, mutex_dict, mutex_ranges, comp_axioms
        )
...
def build_sas_operator(
    ...
    for var in effects_by_variable:
        ...
        for post, eff_conditions in effects_by_variable[var].items():
            ...
            if pre == post:
                continue
            eff_condition_lists = [sorted(eff_cond.items()) for eff_cond in eff_conditions]
            if ranges[var] == 2:
                ...
                if options.add_implied_preconditions and pre == -1 and (var, 1 - post) in implied_precondition:
...
def translate_strips_axiom(axiom, dictionary, ranges, num_dict, mutex_dict, mutex_ranges, comp_ax):
    conditions = translate_strips_conditions(
        axiom.condition, dictionary, ranges, num_dict, mutex_dict, mutex_ranges, comp_ax
    )
    if conditions is None:
        return []
...
def translate_strips_axioms(axioms, strips_to_sas, ranges, num_dict, mutex_dict, mutex_ranges, comp_ax):
    result = []
    for axiom in axioms:
        sas_axioms = translate_strips_axiom(axiom, strips_to_sas, ranges, num_dict, mutex_dict, mutex_ranges, comp_ax)
        result.extend(sas_axioms)
    return result


def add_key_to_comp_axioms(
    comparison_axioms, translation_key
):  # Adds a "human" readable name to each propositional variable
    for axiom in comparison_axioms[1]:
        value_list = []
        assert axiom.effect == len(translation_key), "current effect %s != next variable %s" % (
            axiom.effect,
            len(translation_key),
        )
...
def translate_task(
    ...
    with timers.timing("Processing axioms", block=True):
        axioms, axiom_init, axiom_layer_dict = axiom_rules.handle_axioms(actions, axioms, goal_list, global_constraint)
    ...
    goal_dict_list = translate_strips_conditions(
        goal_list, strips_to_sas, ranges, numeric_strips_to_sas, mutex_dict, mutex_ranges, comparison_axioms
    )
    global_constraint_dict_list = translate_strips_conditions(
        [global_constraint], strips_to_sas, ranges, numeric_strips_to_sas, mutex_dict, mutex_ranges, comparison_axioms
    )
    ...
    axioms = translate_strips_axioms(
        axioms, strips_to_sas, ranges, numeric_strips_to_sas, mutex_dict, mutex_ranges, comparison_axioms
    )
...
def trivial_task(solvable):
    variables = sas_tasks.SASVariables([2], [-1], [["Atom dummy(val1)", "Atom dummy(val2)"]], 0)
...
def pddl_to_sas(task):
    ...
    if DEBUG:
        print("Task converted to SAS.")
    #    task.function_administrator.dump()
    if DEBUG:
        print("Relaxed_reachable: %s" % relaxed_reachable)
    if DEBUG:
        print("List of %d Atoms:" % len(atoms))
    #     for atom in atoms:
    #         atom.dump()
    if DEBUG:
        print("List of %d numeric Fluents:" % len(num_fluents))
    #     for fluent in num_fluents:
    #         fluent.dump()
    if len(num_fluents) == 0:
        assert False
    if DEBUG:
        print("List of %d Actions:" % len(actions))
    #     for action in actions:
    #         print(action.name)
    if DEBUG:
        print("List of %d propositional Axioms:" % len(axioms))
    #    if len(axioms) > 0:
    #        axioms[0].dump()
    ...
    #        axiom.dump()
    if DEBUG:
        print("List of %d numeric Axioms:" % len(num_axioms))
    #     for numax in num_axioms:
    #         numax.dump()
    if DEBUG:
        print("List of %d constant predicates from initial state" % len(init_constant_predicates))
    #     for icp in init_constant_predicates:
    #         icp.dump()
    if DEBUG:
        print("List of %d constant numeric predicates from initial state" % len(init_constant_numerics))
    #     for icn in init_constant_numerics:
    #         icn.dump()
    if DEBUG:
        print("List of %d reachable Action Parameters:" % len(reachable_action_params))
    ...
    with timers.timing("Handling numeric axioms"):
        num_axioms_by_layer, max_num_layer, num_axiom_map, const_num_axioms = numeric_axiom_rules.handle_axioms(
            num_axioms
        )
...
def dump_statistics(sas_task):
    ...
    print(
        ("Translator derived variables: %d" % len([layer for layer in sas_task.variables.axiom_layers if layer >= 0]))
    )
    ...
    print(("Translator total mutex groups size: %d" % sum(mutex.get_encoding_size() for mutex in sas_task.mutexes)))
