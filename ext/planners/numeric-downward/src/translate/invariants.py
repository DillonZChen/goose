...
class InvariantPart:
    ...
    def get_assignment(self, parameters, literal):
        equalities = [(arg, literal.args[argpos]) for arg, argpos in zip(parameters, self.order)]
...
class Invariant:
    ...
    def operator_too_heavy(self, h_action):
        add_effects = [
            ...
            and not eff.peffect.negated
            and self.predicate_to_part.get(eff.peffect.predicate)
        ]
    ...
    def operator_unbalanced(self, action, enqueue_func):
        ...
        relevant_effs = [
            eff
            for eff in action.effects
            if isinstance(eff.peffect, pddl.Literal) and self.predicate_to_part.get(eff.peffect.predicate)
        ]
        add_effects = [eff for eff in relevant_effs if not eff.peffect.negated]
        del_effects = [eff for eff in relevant_effs if eff.peffect.negated]
    ...
    def refine_candidate(self, add_effect, action, enqueue_func):
        ...
        for del_eff in [
            eff for eff in action.effects if isinstance(eff.peffect, pddl.Literal) and eff.peffect.negated
        ]:
    ...
    def imply_del_effect(self, del_effect, lhs_by_pred):
        ...
        for literal in itertools.chain(get_literals(del_effect.condition), [del_effect.peffect.negate()]):
            poss_assignments = []
