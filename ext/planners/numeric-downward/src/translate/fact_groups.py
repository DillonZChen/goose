...
def build_translation_key(groups):
    ...
    for group in groups:
        ...
        translation_keys.append(group_key)
...
def sort_groups(groups):
    ...
    return sorted((sorted(group, key=lambda x: repr(x)) for group in groups), key=lambda x: repr(x))
...
def compute_groups(task, atoms, reachable_action_params):
    ...
    #    print ("fact_groups sorted groups")
    #    for group in groups:
    #        print(group)
