...
## this class deals with propositional axioms. Numeric axioms are treated in numeric_axiom_rules.py


def handle_axioms(operators, axioms, goal_list, global_constraint):
    axioms_by_atom = get_axioms_by_atom(axioms)
    axiom_literals = compute_necessary_axiom_literals(axioms_by_atom, operators, goal_list, global_constraint)
...
def compute_necessary_axiom_literals(axioms_by_atom, operators, goal_list, global_constraint):
    ...
    # Initialize queue with axioms required for goal_list and operators.
    register_literals(goal_list, False)
    register_literals([global_constraint], False)
...
def simplify(axioms):
    ...
    for axiom in axioms:
        axiom.condition.sort()  ## HACK: I commented out the __le__ in conditions.Literal as FunctionComparison objects
        ## from NFD do not have a key. So axioms are now sorted according to hash value and not
        ## by key. Not sure if this obsoletes some simplifications...
