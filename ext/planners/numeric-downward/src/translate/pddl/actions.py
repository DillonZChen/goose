...
from . import conditions, effects, f_expression, pddl_types


class Action(object):
    def __init__(self, name, parameters, num_external_parameters, precondition, effects, cost):
    ...
    def parse(alist):
        ...
        if parameters_tag_opt == ":parameters":
            parameters = pddl_types.parse_typed_list(next(iterator), only_variables=True)
    ...
    def instantiate(
        ...
        arg_list = [var_mapping[par.name] for par in self.parameters[: self.num_external_parameters]]
...
class PropositionalAction:
    def __init__(self, name, precondition, effects, cost):
        ...
        self.assign_effects = (
            []
        )  # NFD action also contain assign effects. Cassic propositional planning tasks with :action-costs contain exactly one assign effect (the increase effect on total-cost)
        ...
        for condition, effect in effects:
            if (
                not isinstance(effect, f_expression.FunctionAssignment)
                and effect.negated
