...
class Condition(object):
    def __init__(self, parts):
        ...
        if DEBUG:
            print("Initialized %s Condition with the following %d parts:" % (self.__class__, len(parts)))
        if DEBUG:
            for p in parts:
                print(p)
    ...
    def uniquify_variables(self, type_map, renamings={}):
        ...
        if not self.parts:
            ...
            return self.__class__([part.uniquify_variables(type_map, renamings) for part in self.parts])
...
class QuantifiedCondition(Condition):
    ...
    def uniquify_variables(self, type_map, renamings={}):
        ...
        new_parameters = [par.uniquify_name(type_map, renamings) for par in self.parameters]
...
class ExistentialCondition(QuantifiedCondition):
    def _untyped(self, parts):
        ...
        return ExistentialCondition(self.parameters, [Conjunction(type_literals + parts)])
...
class FunctionComparison(Condition):  # NFD class yielding the truth value of a function comparison
    ...
    def __str__(self):
        return "%s (%s %s)" % (self.__class__.__name__, self.comparator, ", ".join(map(str, self.parts)))

    def __repr__(self, *args, **kwargs):
        return self.__str__

    def uniquify_variables(self, type_map, renamings={}):
        return self.__class__(self.comparator, [part.rename_variables(renamings) for part in self.parts])
    ...
    def instantiate(
        ...
        instantiated_parts = [
            part.instantiate(var_mapping, fluent_functions, init_function_vals, task, new_axiom) for part in self.parts
        ]
