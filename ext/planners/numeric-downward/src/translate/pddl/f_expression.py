...
class FunctionalExpression(object):
    ...
    def _postorder_visit(self, method_name, *args):
        part_results = [part._postorder_visit(method_name, *args) for part in self.parts]
    ...
    def compile_objectfunctions_aux(self, used_variables, recurse_object_terms=True):
        ...
        new_parts = []
        for part in self.parts:
            typed, parts, new_part = part.compile_objectfunctions_aux(used_variables, recurse_object_terms)
            typed_vars += typed
            conjunction_parts += parts
            new_parts.append(new_part)
        return (typed_vars, conjunction_parts, self.__class__(new_parts))

    def instantiate(self, var_mapping, fluent_functions, init_function_vals, task, new_axioms=[]):
...
class ArithmeticExpression(FunctionalExpression):
    ...
    def rename_variables(self, renamings={}):
        return self.__class__([part.rename_variables(renamings) for part in self.parts])
...
class PrimitiveNumericExpression(FunctionalExpression):
    ...
    def __eq__(self, other):
        return self.__class__ == other.__class__ and self.symbol == other.symbol and self.args == other.args
    ...
    def instantiate(
        self, var_mapping, fluent_functions, init_function_vals, task, new_axioms
    ):  # =set()): ## removed default constructor for debug reasons
        ...
        if fluent_functions != None:
            if pne not in fluent_functions and not pne.symbol.startswith("derived!"):
                ...
                new_axiom = new_axiom.instantiate(var_mapping, fluent_functions, init_function_vals, task, new_axioms)
...
class FunctionAssignment(object):
    ...
    def rename_variables(self, renamings):  # NFD: FunctionAssignments are no longer restricted to "total-cost"
        return self.__class__(self.fluent.rename_variables(renamings), self.expression.rename_variables(renamings))
    ...
    def instantiate(
        ...
        fluent = self.fluent.instantiate(var_mapping, fluent_functions, init_function_vals, task, new_axioms)
        try:
            expression = self.expression.instantiate(
                var_mapping, fluent_functions, init_function_vals, task, new_axioms
            )
    ...
    def instantiate_cost(self, var_mapping, fluent_functions, init_function_vals, task):
        ...
        try:
            ...
            if isinstance(self.expression, PrimitiveNumericExpression):
                ...
                instantiated_pne = PrimitiveNumericExpression(
                    self.expression.symbol, args
                )  # instantiates the current PNE
                expression = init_function_vals[
                    instantiated_pne
                ]  # assumes that the expression of the PNE is a NumericConstant
                # defined in the initial state in the problem file of an PDDL task.
            assert isinstance(
                expression, NumericConstant
            )  # either expression was a NumericConstant defined in the operator already,
