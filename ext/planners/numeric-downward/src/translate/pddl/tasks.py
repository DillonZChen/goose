...
from . import actions, axioms, conditions, f_expression, functions, pddl_types, predicates
...
class Task(object):
    ...
    def add_global_constraints(
        self,
    ):  # for each global constraint axioms add a new universal axiom "forall PARAMETERS satisfy AXIOM"
        # and finally add a conjunction axiom of all such universal constraints "AND UNIVERSALAXIOMS"
        if DEBUG:
            print("Adding global constraints")
        ...
        if universal_constraints:
            if DEBUG:
                print("There are %d universal constraints" % len(universal_constraints))
            the_global_constraint = conditions.Conjunction(universal_constraints)
        if DEBUG:
            print("Adding axiom for global constraint")
        self.add_axiom([], the_global_constraint)
        self.global_constraint = conditions.Atom(self.axioms[-1].name, [])
        if DEBUG:
            print("The global constraint is : %s" % self.global_constraint)
...
class DerivedFunctionAdministrator(object):
    ...
    def get_derived_function(self, exp):
        ...
        assert isinstance(exp, f_expression.FunctionalExpression), (
            "expected Functional Expression, got %s " % exp.__class__
        )
        if isinstance(exp, f_expression.PrimitiveNumericExpression):
            ...
            if DEBUG:
                print("arithmetic expression ", exp)
            assert isinstance(exp, f_expression.ArithmeticExpression)
            if len(exp.parts) != 2:
                assert exp.op in ["+", "*"]
            if exp.op in ("+", "*"):
                if DEBUG:
                    print("DEBUG tasks.py checking parts of exp ", exp)
                if DEBUG:
                    print("DEBUG tasks.py exp.parts befor %s" % [exp.parts])
                exp.parts = tuple(
                    sorted(list(exp.parts))
                )  # as addition and multiplication are commutative, we only generate one derived expression
                if DEBUG:
                    print("DEBUG tasks.py exp.parts after %s" % [exp.parts])
            ...
            if key not in self.functions:
                ...
                for df in keylist[1:]:
                    pnelist.append(
                        f_expression.PrimitiveNumericExpression(
                            df.symbol, default_args[argindex : argindex + len(df.args)], "D"
                        )
                    )
                    if DEBUG:
                        print("DEBUG tasks.py appending to pnelist %s" % pnelist[-1])
        ...
        if DEBUG:
            print("returning derived function ", pne_symbol)
