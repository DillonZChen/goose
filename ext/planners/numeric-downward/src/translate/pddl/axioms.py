...
class Axiom(object):
    def __init__(self, name, parameters, num_external_parameters, condition, is_global_constraint=False):
        ...
        self.is_global_constraint = (
            is_global_constraint  # in tasks.add_global_constraints this flag is read to create "the" global constraint
        )
    ...
    def parse(alist):
        ...
        return Axiom(predicate.name, predicate.arguments, len(predicate.arguments), condition)
    ...
    def instantiate(
        self, var_mapping, init_facts, fluent_facts, init_function_vals, fluent_functions, task, new_axiom, new_modules
    ):
        # The comments for Action.instantiate apply accordingly.
        arg_list = [self.name] + [var_mapping[par.name] for par in self.parameters[: self.num_external_parameters]]
        ...
        effect_args = [var_mapping.get(arg.name, arg.name) for arg in self.parameters[: self.num_external_parameters]]
    ...
    def instantiate(self, var_mapping, fluent_functions, init_function_vals, task, new_constant_axioms):
        ...
        for part in self.parts:
            if isinstance(part, f_expression.NumericConstant):
                ...
                parts.append(
                    part.instantiate(var_mapping, fluent_functions, init_function_vals, task, new_constant_axioms)
                )
