...
class Effect(object):
    ...
    def __eq__(self, other):
        return (
            ...
            and self.peffect == other.peffect
        )
        ...
        if isinstance(norm_effect, ConjunctiveEffect):
            ...
            for effect in norm_effect.effects:
                assert (
                    ...
                    or isinstance(effect, NumericEffect)
                )
    ...
    def normalize(self):
        ...
        if isinstance(norm_effect, ConjunctiveEffect):
            ...
            for effect in norm_effect.effects:
                ...
                ), ("Instance is %s instead" % effect.__class__)
    ...
    def extract_cost(self):  # returns pair (cost_effect, regular_effect)
        if self.effect.is_cost_assignment():
            return (
                self,
                self,
            )  # A cost effect is still a regular effect and using the old cost is deprecated but still supported.
        else:
            return None, self
