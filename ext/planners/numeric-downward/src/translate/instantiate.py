...
def init_function_values(init_facts):
    assignments = [func_assign for func_assign in init_facts if isinstance(func_assign, pddl.FunctionAssignment)]
    init_values = {}
...
def instantiate(task, model):
    ...
    for atom in model:
        if isinstance(atom.predicate, pddl.Action):
            ...
            variable_mapping = dict([(par.name, arg) for par, arg in zip(parameters, atom.args)])
            ...
            variable_mapping = dict([(par.name, arg) for par, arg in zip(axiom.parameters, atom.args)])
            ...
            axiom = atom.predicate
            variable_mapping = dict([(par, arg) for par, arg in zip(axiom.parameters, atom.args)])
            inst_axiom = axiom.instantiate(
                variable_mapping, fluent_functions, init_function_vals, task, new_constant_numeric_axioms
            )
...
def explore(task):
    ...
    #     prog.dump()
    if DEBUG:
        print("DEBUG: Exploring Task Step [2]: build model 'model'")
    model = build_model.compute_model(prog)
    #     print("instantiate.explore task dumps task")
    #     task.dump()
    if DEBUG:
        print("DEBUG: Exploring Task Step [3]: instantiate model")
