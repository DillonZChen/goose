...
def identify_constants(axioms, axiom_by_pne):
    def is_constant(axiom):
        if DEBUG:
            print("Testing if %s is constant" % axiom)
        if isinstance(axiom, pddl.PrimitiveNumericExpression):
            if axiom in axiom_by_pne:
                axiom = axiom_by_pne[axiom]
            else:
                return (False, None)
        if axiom.op == None and isinstance(axiom.parts[0], pddl.NumericConstant):
            ...
            for part in axiom.parts:
                if DEBUG:
                    print("recursively checking part %s" % part)
                const, val = is_constant(part)
                if not const:
                    if DEBUG:
                        print("not constant -> aborting loop")
                    all_constants = False
                    break
                if DEBUG:
                    print("appending constant %s" % val)
                values.append(val)
            if all_constants:
