...
class SASTask:
    ...
    def __init__(
        ...
        self.metric = (
            metric  # tuple containing '<' : minimize or '>' : maximize and the index of the (numeric) metric variable
        )
    ...
    def validate(self):
        ...
        assert isinstance(self.global_constraint, tuple), "%s of type %s" % (
            self.global_constraint,
            type(self.global_constraint),
        )
...
class SASVariables:
    ...
    def dump(self):
        for var, (rang, names, axiom_layer) in enumerate(zip(self.ranges, self.value_names, self.axiom_layers)):
            if axiom_layer != -1:
...
class SASInit:
    ...
    def validate(self, variables):
        ...
        assert len(self.values) == len(variables.ranges), "length of values= %s length of varranges= %s" % (
            len(self.values),
            len(variables.ranges),
        )
...
class SASOperator:
    ...
    def dump(self):
        ...
        print(
            "Cost:"
        )  # Cost stores the evaluated value of the metric expression (for state dependent action costs this field stores the expression evaluated in the initial state)
        print("  %f" % self.cost)
...
class SASCompareAxiom:
    ...
    def pretty_print(self, numeric_variables):
        print(
            ...
            % (
                self.comp,
                numeric_variables.variable_names[self.parts[0]],
                numeric_variables.variable_names[self.parts[1]],
            )
        )  # ,variables.value_names[self.effect][0]))

    def dump(self):
        values = (self.effect, self.comp, " ".join([str(var) for var in self.parts]))
...
class SASNumericAxiom:  # takes an arithmetic operator ("+", "-", "*", "/") and 3 numeric fluents where the 3rd represents the computation of the first two.
    ...
    def dump(self):
        values = (self.effect, self.op, " ".join([str(var) for var in self.parts]))
