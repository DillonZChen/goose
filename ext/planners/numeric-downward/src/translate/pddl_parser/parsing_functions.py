...
def parse_typed_list(alist, only_variables=False, constructor=pddl.TypedObject, default_type="object"):
    ...
    while alist:
        ...
        for item in items:
            assert not only_variables or item.startswith("?"), "Expected item to be a variable: %s in (%s)" % (
                item,
                " ".join(items),
            )
...
def parse_condition_aux(alist, negated, type_dict, predicate_dict):
    ...
    if tag == "imply":
        parts = [
            parse_condition_aux(args[0], not negated, type_dict, predicate_dict),
...
def is_function_comparison(alist):
    ...
    if isinstance(symbol, list):
        if symbol[0] in ("+", "/", "*", "-"):
            return True
        symbol = symbol[0]
    if pddl.Task.FUNCTION_SYMBOLS.get(symbol, "object") == "number" or symbol.replace(".", "").isdigit():
    ...
    if isinstance(tmp_effect, pddl.ConjunctiveEffect):
        ...
        if isinstance(tmp_effect, pddl.UniversalEffect):
            ...
            if isinstance(tmp_effect.effect, pddl.ConditionalEffect):
                ...
                assert isinstance(tmp_effect.effect.effect, pddl.SimpleEffect) or isinstance(
                    tmp_effect.effect.effect, pddl.NumericEffect
                )
                effect = tmp_effect.effect.effect.effect
            else:
                assert isinstance(tmp_effect.effect, pddl.SimpleEffect) or isinstance(
                    tmp_effect.effect, pddl.NumericEffect
                ), ("Effect is not primitive but %s" % tmp_effect.effect.effect.__class__)
...
def parse_effect(alist, type_dict, predicate_dict):
    ...
    tag = alist[0]
    if tag == "and":
        return pddl.ConjunctiveEffect([parse_effect(eff, type_dict, predicate_dict) for eff in alist[1:]])
    elif tag == "forall":
        assert len(alist) == 3
        parameters = parse_typed_list(alist[1])
    ...
    if effect_list:
        try:
            ...
            if cost is None and eff:
                if DEBUG:
                    print("adding artificial effect (increase total-cost 1) to %s" % name)
...
def parse_global_constraint(alist, type_dict, predicate_dict):  # global constraints are parsed as special Axioms
    if DEBUG:
        print("parsing constraint %s" % [alist])
    iterator = iter(alist)
    action_tag = next(iterator)
...
def parse_task(domain_pddl, task_pddl):
    ...
    task_name, task_domain_name, task_requirements, objects, init, num_init, goal, metric = parse_task_pddl(
    ...
    check_for_duplicates(  # This will remove duplicates now instead of outright aborting.
        objects, errmsg="error: duplicate object %r", finalmsg="please check :constants and :objects definitions"
    )
...
def parse_domain_pddl(domain_pddl):
    ...
    the_predicates = [pddl.Predicate("=", [pddl.TypedObject("?x", "object"), pddl.TypedObject("?y", "object")])]
    #    the_free_functions = [] ## support for global constraints with free functions is not implemented yet
    correct_order = [":requirements", ":types", ":constants", ":predicates", ":functions"]  # , ":free_functions"]
    seen_fields = []
    first_action = None
    for opt in iterator:
        ...
        if field == ":requirements":
            ...
            the_functions = parse_typed_list(opt[1:], constructor=parse_function, default_type="number")
...
def parse_task_pddl(task_pddl, type_dict, predicate_dict):
    ...
    for fact in init[1:]:
        ...
        if fact[0] == "=":
            ...
            if not isinstance(assignment.expression, pddl.NumericConstant):
                raise SystemExit("Illegal assignment in initial state " + "specification:\n%s" % assignment)
            if assignment.fluent in initial_assignments:
                ...
                if assignment.expression == prev.expression:
                    print("Warning: %s is specified twice" % assignment, "in initial state specification")
                else:
                    raise SystemExit(
    ...
    if DEBUG:
        initial.sort(key=lambda x: x.__repr__())  # makes the result deterministic
