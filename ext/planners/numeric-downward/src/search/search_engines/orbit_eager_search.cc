...
OrbitEagerSearch::OrbitEagerSearch(const Options &opts)
    : SearchEngine(opts),
    ...
    if (g_symmetry_graph)
      g_symmetry_graph->initialize(task);
    else {
...
void OrbitEagerSearch::initialize() {
...
void OrbitEagerSearch::print_checkpoint_line(int g) const {
    cout << "[g=" << g << ", ";
    statistics.print_basic_statistics();
    cout << "]" << endl;
}

void OrbitEagerSearch::print_statistics() const {
    statistics.print_detailed_statistics();
    search_space.print_statistics();
    pruning_method->print_statistics();
}

SearchStatus OrbitEagerSearch::step() {
...
pair<SearchNode, bool> OrbitEagerSearch::fetch_next_node() {
...
void OrbitEagerSearch::reward_progress() {
    // Boost the "preferred operator" open lists somewhat whenever
    // one of the heuristics finds a state with a new best h value.
    open_list->boost_preferred();
}

void OrbitEagerSearch::dump_search_space() const {
    search_space.dump();
}

void OrbitEagerSearch::start_f_value_statistics(EvaluationContext &eval_context) {
    if (f_evaluator) {
...
void OrbitEagerSearch::update_f_value_statistics(const SearchNode &node) {
    if (f_evaluator) {
        /*
...
static SearchEngine *_parse(OptionParser &parser) {
    parser.document_synopsis("Orbit eager best-first search", "");
    ...
    OrbitEagerSearch *engine = nullptr;
    if (!parser.dry_run()) {
        opts.set<bool>("mpd", false);
        engine = new OrbitEagerSearch(opts);
    }
...
static SearchEngine *_parse_greedy(OptionParser &parser) {
    parser.document_synopsis("Orbit greedy search (eager)", "");
    parser.document_note(
        "Open list",
...
static Plugin<SearchEngine> _plugin("orbit_eager", _parse);
static Plugin<SearchEngine> _plugin_astar("orbit_astar", _parse_astar);
static Plugin<SearchEngine> _plugin_greedy("orbit_eager_greedy", _parse_greedy);
}
