...
enum LPVariableType {
    continous = 0, binary = 1, integer = 2
};

void add_lp_solver_option_to_parser(options::OptionParser &parser);
void add_lp_constraint_option_to_parser(options::OptionParser &parser);
...
struct LPVariable {
    ...
    LPVariable(double lower_bound_,
               double upper_bound_,
               double objective_coefficient_,
               std::string name_, LPVariableType type_);
...
public:
    LPConstraintType lp_type;
    LP_METHOD(explicit LPSolver(LPSolverType solver_type, LPConstraintType lp_type))
    ...
    */
    ...
    LP_METHOD(void set_time_limit(double time) const)
};
#ifdef __GNUG__
