...
void add_lp_solver_option_to_parser(OptionParser &parser) {
    ...
    lp_solvers.push_back("IPCPLEX");
    lp_solvers_doc.push_back("commercial IP solver by IBM");
    ...
    void add_lp_constraint_option_to_parser(OptionParser &parser) {
        parser.document_note(
                             "Note",
                             "add lp solver options");
        vector<string> lp_constraints;
        vector<string> lp_constraints_doc;
        lp_constraints.push_back("lp");
        lp_constraints_doc.push_back("lp relaxation");
        lp_constraints.push_back("ip");
        lp_constraints_doc.push_back("ip solution");
        parser.add_enum_option(
                               "lprelaxation",
                               lp_constraints,
...
LPVariable::LPVariable(double lower_bound_, double upper_bound_,
    ...
    : lower_bound(lower_bound_),
      upper_bound(upper_bound_),
      objective_coefficient(objective_coefficient_), type(integer) {
}

    LPVariable::LPVariable(double lower_bound_, double upper_bound_,
                           double objective_coefficient_, string name_)
    : lower_bound(lower_bound_),
    upper_bound(upper_bound_),
    objective_coefficient(objective_coefficient_), name(name_), type(integer) {
    }

    LPVariable::LPVariable(double lower_bound_, double upper_bound_,
                           double objective_coefficient_, string name_, LPVariableType type_)
    : lower_bound(lower_bound_),
    upper_bound(upper_bound_),
    objective_coefficient(objective_coefficient_), name(name_), type(type_) {
    }
...
LPSolver::LPSolver(LPSolverType s_t, LPConstraintType c_t)
...
LPSolver::LPSolver(LPSolverType s_t)
    : is_initialized(false),
    is_solved(false),
    num_permanent_constraints(0),
    has_temporary_constraints_(false),
    solver_type(s_t),
    lp_type(LPConstraintType::LP) {
        lp_solver = create_lp_solver(solver_type);
    }
...
void LPSolver::load_problem(LPObjectiveSense sense,
    ...
    try {
        ...
        if (lp_type == LPConstraintType::IP){
            //cout << "setting variables " << variables.size() << " " << lp_solver->getNumCols() << endl;
            int iType = 0;
            for (const LPVariable &var : variables) {
                //cout << iType << endl;
                if (var.type == integer)
                    lp_solver->setInteger(iType);
...
//        s << id++;
//        //cout << s.str() << endl;
//        lp_solver->writeLp(s.str().c_str());
        if (lp_type == LPConstraintType::IP){
            lp_solver->branchAndBound();
            //cout << " ** ip " << lp_solver->getObjValue() << endl;
        }else{
            if (is_initialized) {
                lp_solver->resolve();
        } else {
...
void LPSolver::print_statistics() const {
    ...
    void LPSolver::set_time_limit(double time) const{
        lp::set_time_limit(lp_solver,solver_type, time);
    }
#endif
}
