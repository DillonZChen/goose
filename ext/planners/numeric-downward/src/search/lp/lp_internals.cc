...
unique_ptr<OsiSolverInterface> create_lp_solver(LPSolverType solver_type) {
...
#endif
        break;
        case LPSolverType::IPCPLEX:
#ifdef COIN_HAS_IPCPX
        {
            OsiIPCpxSolverInterface *cpx_solver = new OsiIPCpxSolverInterface;
            CPXsetintparam(cpx_solver->getEnvironmentPtr(), CPX_PARAM_THREADS, 1);
            cpx_solver->passInMessageHandler(new ErrorCatchingCoinMessageHandler);
            lp_solver = cpx_solver;
        }
#else
            missing_symbol = "COIN_HAS_IPCPX";
#endif
            break;
    default:
    ...
    void  set_time_limit(std::unique_ptr<OsiSolverInterface> const & lp_solver, LPSolverType solver_type, double time) {
        ...
        switch (solver_type) {
            ...
            case LPSolverType::CPLEX:
#ifdef COIN_HAS_CPX
                CPXsetdblparam((dynamic_cast<OsiCpxSolverInterface*>(lp_solver.get()))->getEnvironmentPtr(), CPX_PARAM_TILIM, time);
#else
                missing_symbol = "COIN_HAS_CPX";
#endif
                break;
