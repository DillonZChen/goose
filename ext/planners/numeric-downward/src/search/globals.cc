...
bool test_goal(const GlobalState &state) {
    ...
    //   int j = g_goal[i].first;
    //   std::cout << j << " " << state[j] << " " << g_goal[i].second << std::endl;
    // }
    // exit(-1);
    for (size_t i = 0; i < g_goal.size(); ++i) {
    ...
    for (const string &fact_name : g_fact_names[i]) {
      if (fact_name != "<none of those>" &&
        ...
        if (fact_name.find("NegatedAtom ") != std::string::npos) {
          name = fact_name.substr(12);
        } else {
          name = fact_name.substr(5);
        }
        if (name.size() > 0) {
          ret.push_back(name);
        }
...
std::vector<std::string> get_fluent_strings() {
  return get_fluent_strings_and_indices().first;
}

std::pair<std::vector<std::string>, std::vector<int>> get_fluent_strings_and_indices() {
  std::vector<std::string> string_ret;
  std::vector<int> index_ret;
  ...
  for (size_t i = 0; i < g_numeric_var_names.size(); ++i) {
    ...
    if (num_name != "<none of those>" &&
      ...
      num_name = num_name.substr(4);
      if (num_name.size() > 0) {
        string_ret.push_back(num_name);
        index_ret.push_back(i);
      }
    }
  }
  return std::make_pair(string_ret, index_ret);
}
