...
from . import call, limits, returncodes, util
...
def run_search(executable, args, sas_file, plan_manager, time, memory):
    ...
    try:
        exitcode = call.check_call(complete_args, stdin=sas_file, time_limit=time, memory_limit=memory)
...
def compute_run_time(timeout, configs, pos):
    ...
    print("config {}: relative time {}, remaining {}".format(pos, relative_time, remaining_relative_time))
...
def run_sat_config(
    configs, pos, search_cost_type, heuristic_cost_type, executable, sas_file, plan_manager, timeout, memory
):
    ...
    args.extend(["--internal-previous-portfolio-plans", str(plan_manager.get_plan_counter())])
    result = run_search(executable, args, sas_file, plan_manager, run_time, memory)
    plan_manager.process_new_plans()
    return result


def run_sat(configs, executable, sas_file, plan_manager, final_config, final_config_builder, timeout, memory):
...
def run_opt(configs, executable, sas_file, plan_manager, timeout, memory):
    for pos, (relative_time, args) in enumerate(configs):
        run_time = compute_run_time(timeout, configs, pos)
        exitcode = run_search(executable, args, sas_file, plan_manager, run_time, memory)
        yield exitcode
...
def run(portfolio, executable, sas_file, plan_manager, time, memory):
    ...
    if "TIMEOUT" in attributes:
        sys.exit(
            "The TIMEOUT attribute in portfolios has been removed. " "Please pass a time limit to fast-downward.py."
        )
    ...
    if optimal:
        ...
        exitcodes = run_sat(
            configs, executable, sas_file, plan_manager, final_config, final_config_builder, timeout, memory
        )
