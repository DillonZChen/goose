...
def get_executable(build, rel_path):
    ...
    if not os.path.exists(build_dir):
        ...
        if not os.path.exists(build_dir):
            raise IOError(
                "Could not find build '{build}' at {build_dir}. " "Please run './build.py {build}'.".format(**locals())
            )

    abs_path = os.path.join(build_dir, rel_path)
    if not os.path.exists(abs_path):
        raise IOError(
            "Could not find '{rel_path}' in build '{build}'. " "Please run './build.py {build}'.".format(**locals())
        )
...
def call_component(executable, options, stdin=None, stdout=True, time_limit=None, memory_limit=None):
    ...
    call.check_call(
        [executable] + options, stdin=stdin, stdout=stdout, time_limit=time_limit, memory_limit=memory_limit
    )
    print
...
def run_search(args):
    ...
    memory_limit = limits.get_memory_limit(args.search_memory_limit, args.overall_memory_limit)
    # print_component_settings(
    ...
    if args.portfolio:
        ...
        portfolio_runner.run(args.portfolio, search, args.search_input, plan_manager, time_limit, memory_limit)
    else:
        if not args.search_options:
            raise ValueError("search needs --alias, --portfolio, or search options")
        if "--help" not in args.search_options:
            args.search_options.extend(["--internal-plan-file", args.plan_file])
        try:
            call_component(
                search, args.search_options, stdin=args.search_input, time_limit=time_limit, memory_limit=memory_limit
            )
...
def run_validate(args):
    ...
    print_component_settings(
        "validate", validate_inputs, [], time_limit=None, memory_limit=VALIDATE_MEMORY_LIMIT_IN_MB
    )
