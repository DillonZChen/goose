...
def preprocess():
    ...
    cmd = ["./fast-downward.py", "--translate", "--preprocess", "misc/tests/benchmarks/gripper/prob01.pddl"]
    assert subprocess.check_call(cmd, cwd=REPO_ROOT_DIR) == 0


def cleanup():
    subprocess.check_call(["./fast-downward.py", "--cleanup"], cwd=REPO_ROOT_DIR)
...
def test_portfolios():
    for name, portfolio in PORTFOLIOS.items():
        cmd = ["./fast-downward.py", "--portfolio", portfolio, "--search-time-limit", "30m", "output"]
        assert run_driver(cmd) in [EXIT_PLAN_FOUND, EXIT_UNSOLVED_INCOMPLETE]
...
def test_time_limits():
    for internal, external, expected_soft, expected_hard in [
        ...
        assert limits._get_soft_and_hard_time_limits(internal, external) == (expected_soft, expected_hard)
