...
def _set_limit(kind, soft, hard=None):
    ...
    try:
        ...
        print(
            ...
            "Previous limit: {}".format(kind, soft, hard, err, resource.getrlimit(kind)),
            file=sys.stderr,
        )
...
def _get_soft_and_hard_time_limits(internal_limit, external_hard_limit):
    ...
    print("time limit %.2f -> (%d, %d)" % (internal_limit, soft_limit, hard_limit))
    sys.stdout.flush()
...
def set_time_limit(time_limit):
    ...
    soft_limit, hard_limit = _get_soft_and_hard_time_limits(time_limit, external_hard_limit)
