...
EXAMPLE_PORTFOLIO = os.path.relpath(aliases.PORTFOLIOS["seq-opt-fdss-1"], start=util.REPO_ROOT_DIR)
...
EXAMPLES = [
    ...
    (
        ...
        ["./fast-downward.py", "--translate", "--preprocess", "misc/tests/benchmarks/gripper/prob01.pddl"],
    ),
    (
    ...
    (
        ...
        ["./fast-downward.py", "--portfolio", EXAMPLE_PORTFOLIO, "--search-time-limit", "30m", "output"],
    ),
    (
...
def _check_mutex_args(parser, args, required=False):
    ...
    if required and not any(is_specified for _, is_specified in args):
        parser.error("exactly one of {%s} has to be specified" % ", ".join(name for name, _ in args))
...
def parse_args():
    ...
    components.add_argument("--preprocess", action="store_true", help="run preprocessor component")
    ...
    driver_other.add_argument("--debug", action="store_true", help="alias for --build=debug32")
    driver_other.add_argument(
    ...
    driver_other.add_argument("--portfolio", metavar="FILE", help="run a portfolio specified in FILE")
    ...
    if args.build and args.debug:
        parser.error("The option --debug is an alias for --build=debug32. " "Do no specify both --debug and --build.")
    if not args.build:
        if args.debug:
            args.build = "debug32"
        else:
