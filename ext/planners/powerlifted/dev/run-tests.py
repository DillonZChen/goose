...
OPTIMAL_PLAN_COSTS = {
    "domains/airport/p05-airport2-p1.pddl": 21,
...
class TestRun:
    ...
    def get_config(self):
        return "{}, {}, {}, and {}".format(self.search, self.heuristic, self.generator, self.state_representation)
    ...
    def run(self):
        ...
        output = subprocess.check_output(
            [
                os.path.join(BASEDIR, "powerlifted.py"),
                "-i",
                os.path.join(BASEDIR, "dev", self.instance),
                "-s",
                self.search,
                "-e",
                self.heuristic,
...
def parse_options():
    ...
    parser.add_argument("--minimal", dest="minimal", action="store_true", help="Use minimal test set.")
