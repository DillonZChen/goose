...
def main():
    ...
    if os.path.isfile(options.output_file):
        print("WARNING: file %s already exists, it will be overwritten" % options.output_file)
...
def print_initial_state(output, task, atom_index, object_index, predicate_index):
    ...
    for index, atom in enumerate(task.init):
        ...
        print(
            atom,
            "%d %d %d %d" % (index, predicate_index[atom.predicate], atom.negated, len(atom.args)),
            " ".join(str(object_index[o]) for o in atom.args),
            file=output,
        )
...
def print_objects(output, task, object_index, type_index, types_dict):
    ...
    for index, obj in enumerate(task.objects):
        ...
        print("%s %d %d" % (obj.name, index, len(types_dict[obj.type_name])), end=" ", file=output)
        print(" ".join(str(type_index[t]) for t in sorted(types_dict[obj.type_name])), file=output)
...
def print_predicates(output, task, predicate_index, type_index):
    ...
    for index, p in enumerate(task.predicates):
        ...
        if not p.static:
            ...
            for arg in p.arguments:
                try:
                    ...
                    raise NotImplementedError(
                        "Your task probably has an " "'either'-typed predicate, " "which is not implemented."
                    )
            ...
            print(" ".join(str(type_index["object"]) for arg in p.arguments), file=output)
