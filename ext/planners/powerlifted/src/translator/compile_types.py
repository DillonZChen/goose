    ...
    for action in task.actions:
        ...
        for cond in precond:
            ...
            for arg in cond.args:
                name = arg
                param_type = next((x.type_name for x in action.parameters if x.name == arg), None)
                if param_type is None:
                    # If the type is none, then it is not a parameter and
                    # it must be constant.  THus, we search for its type in the
                    # obj list.
                    param_type = next((x.type_name for x in task.objects if x.name == arg), None)
                obj_in_action.add((param_type, name))
        literals_in_effects = action.get_literals_in_effects
        for l in literals_in_effects:
            name = l
            param_type = next((x.type_name for x in action.parameters if x.name == l), None)
            if param_type is None:
                # If the type is none, then it is not a parameter and
                # it must be constant.  THus, we search for its type in the
                # obj list.
                param_type = next((x.type_name for x in task.objects if x.name == l), None)
            obj_in_action.add((param_type, name))
