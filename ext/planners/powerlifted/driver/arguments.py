...
def parse_options():
    ...
    parser.add_argument("-d", "--domain", dest="domain", action="store", default=None, help="Domain file in PDDL")
    parser.add_argument(
        "-i", "--instance", dest="instance", action="store", default=None, help="Instance file in PDDL", required=True
    )
    parser.add_argument("--build", dest="build", action="store_true", help="Build planner before search.")
    parser.add_argument("--debug", dest="debug", action="store_true", help="Run planner in debug mode.")
    parser.add_argument("--cxx-compiler", default="default", help="Path to CXX compiler used by CMake.")
    parser.add_argument(
        "-s",
        "--search",
        dest="search",
    ...
    parser.add_argument("-m", "--model_path", default="FilePathUndefined", help="Path to the wlgoose model file")
    parser.add_argument(
        "-g",
        "--generator",
        dest="generator",
        action="store",
        default="yannakakis",
        help="Successor generator method",
    ...
    parser.add_argument(
        ...
        help="Successor generator method",
        default="sparse",
        choices=("sparse", "extensional"),
    )
    parser.add_argument("--seed", action="store", help="Random seed.", default=1)
    parser.add_argument("--time-limit", action="store", type=int, help="Time limit in seconds.", default=1800)
    parser.add_argument(
    ...
    parser.add_argument("--plan-file", dest="plan_file", default="plan", help="name of plan file")
    parser.add_argument(
        "--datalog-file", dest="datalog_file", default="model.lp", help="Datalog model for the lifted heuristic."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--keep-action-predicates", action="store_true", help="flag if the Datalog model should keep action predicates"
    )
    parser.add_argument(
        "--keep-duplicated-rules",
        action="store_true",
        help="flag if the Datalog model should keep duplicated auxiliary rules",
    )
    parser.add_argument(
        "--add-inequalities", action="store_true", help="flag if the Datalog model should add inequalities to rules"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--unit-cost", action="store_true", help="flag if the actions should be treated as unit-cost actions"
    )
    parser.add_argument(
        "--validate", action="store_true", help="flag if VAL should be called to validate the plan found"
    )
    ...
    if args.iteration is not None:
        print(
            "WARNING: You are using at least one iterated search. Values passed to '-s', '-e', and '-g' will be ignored."
        )
