...
TITLE_WHITE_LIST = (
    r"[\w\+-]"  # match 'word characters' (including '_'), '+', and '-'
)
...
def insert_wiki_links(text, titles):
    ...
    re_link = (
        r"(?P<before>\W)(?P<link>%s)(#(?P<anchor>"
        + TITLE_WHITE_LIST
        + r"+))?(?P<after>\W)"
    )
...
def build_planner(build):
    subprocess.check_call(
        [sys.executable, "build.py", build, "downward"], cwd=REPO_ROOT_DIR
    )
...
def get_pages_from_planner(build):
    ...
    pagesplitter = re.compile(
        r">>>>CATEGORY: ([\w\s]+?)<<<<(.+?)>>>>CATEGORYEND<<<<", re.DOTALL
    )
    pages = dict()
...
if __name__ == "__main__":
    ...
    old_doc_titles = [
        title for title in old_titles if title.startswith(DOC_PREFIX)
    ]
