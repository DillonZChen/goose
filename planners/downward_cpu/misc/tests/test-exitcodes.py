...
SEARCH_TESTS = [
    ...
    (
        "strips",
        [],
        "astar(landmark_sum(lm_hm()))",
        defaultdict(lambda: returncodes.SUCCESS),
    ),
    (
        "strips",
        [],
        "astar(landmark_cost_partitioning(lm_hm()))",
        defaultdict(lambda: returncodes.SUCCESS),
    ),
    ...
    (
        "axioms",
        [],
        "astar(landmark_cost_partitioning(lm_zg()))",
        defaultdict(lambda: returncodes.SEARCH_UNSUPPORTED),
    ),
    ...
    ("cond-eff", [], "astar(add())", defaultdict(lambda: returncodes.SUCCESS)),
    ("cond-eff", [], "astar(hm())", defaultdict(lambda: returncodes.SUCCESS)),
    (
        "cond-eff",
        [],
        "astar(ipdb())",
    ...
    (
        "large",
        ["--search-memory-limit", "100M"],
        MERGE_AND_SHRINK,
        defaultdict(
            lambda: returncodes.SEARCH_OUT_OF_MEMORY,
            darwin=returncodes.DRIVER_UNSUPPORTED,
            win32=returncodes.DRIVER_UNSUPPORTED,
        ),
    ),
    ...
    (
        "large",
        ["--search-time-limit", "1s"],
        MERGE_AND_SHRINK,
        defaultdict(
            lambda: returncodes.SEARCH_OUT_OF_TIME,
            win32=returncodes.DRIVER_UNSUPPORTED,
        ),
    ),
]
...
def test_translator_exit_codes(
    task_type, driver_options, translate_options, expected
):
...
def test_search_exit_codes(
    task_type, driver_options, search_options, expected
):
    sas_file = get_sas_file_name(task_type)
    cmd = (
        [sys.executable, DRIVER]
