...
def parse_args():
    ...
    parser.add_argument("benchmarks_dir", help="path to benchmark directory")
    parser.add_argument(
        "suite",
        nargs="*",
        default=["first"],
...
def translate_task(task_file):
    ...
    try:
        output = subprocess.check_output(
            cmd, encoding=sys.getfilesystemencoding()
        )
...
def _get_all_tasks_by_domain(benchmarks_dir):
    ...
    domains = [
        domain_dir
        for domain_dir in benchmarks_dir.iterdir()
        if domain_dir.is_dir()
        and not str(domain_dir.name).startswith((".", "_", "unofficial"))
        and str(domain_dir.name) not in blacklisted_domains
    ]
    ...
    for task in args.suite:
        if task == "first":
            ...
            suite.extend(
                [
                    tasks[0]
                    for tasks in _get_all_tasks_by_domain(args.benchmarks_dir)
                ]
            )
        elif task == "all":
