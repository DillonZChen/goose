...
CONFIGS_NOLP.update(
    configs.default_configs_satisficing(core=True, extended=True)
)
...
@pytest.mark.parametrize(
    "config", sorted(configs.configs_optimal_lp(lp_solver="cplex").values())
)
@pytest.mark.parametrize("debug", [False, True])
...
@pytest.mark.parametrize(
    "config", sorted(configs.configs_optimal_lp(lp_solver="soplex").values())
)
@pytest.mark.parametrize("debug", [False, True])
