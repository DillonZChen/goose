...
EagerSearch::EagerSearch(const plugins::Options &opts)
    : SearchAlgorithm(opts), reopen_closed_nodes(opts.get<bool>("reopen_closed")),
...
void EagerSearch::initialize() {
  log << "Conducting best first search" << (reopen_closed_nodes ? " with" : " without")
...
SearchStatus EagerSearch::step() {
  ...
  */
  ...
  for (const shared_ptr<Evaluator> &preferred_operator_evaluator : preferred_operator_evaluators) {
    collect_preferred_operators(eval_context, preferred_operator_evaluator.get(),
                                preferred_operators);
  }
  ...
  for (OperatorID op_id : applicable_ops) {
    ...
    if (succ_node.is_new()) {
      ...
      if (reopen_closed_nodes) {
        ...
        EvaluationContext succ_eval_context(succ_state, succ_node.get_g(), is_preferred,
                                            &statistics);
...
void EagerSearch::dump_search_space() const { search_space.dump(task_proxy); }
