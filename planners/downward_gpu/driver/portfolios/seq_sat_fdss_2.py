        ...
        if arg == "--search":
            ...
            for weight in (5, 3, 2, 1):
                if orig_search.startswith("lazy"):
                    sub_search = (
                        "lazy_wastar([h],preferred=[h],w=%d,cost_type=S_COST_TYPE)"
                        % weight
                    )
                else:
                    sub_search = (
                        "eager(single(sum([g(),weight(h,%d)])),preferred=[h],cost_type=S_COST_TYPE)"
                        % weight
                    )
                sub_searches.append(sub_search)
            sub_search_string = ",".join(sub_searches)
            iterated_search = (
                "iterated([%s],bound=BOUND,repeat_last=true)"
                % sub_search_string
            )
            new_search = successful_args[pos + 1].replace(
                orig_search, iterated_search
            )
