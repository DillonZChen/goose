...
def getRegexes():
    ...
    bank["macros"] = re.compile(
        r"%%%%(?P<name>%s)\b(\((?P<fmt>.*?)\))?" % ("|".join(MACROS.keys())),
        re.I,
    )
...
class ConfigMaster:
    ...
    def sanity(self, config, gui=0):
        ...
        # Checking suicide
        if (
            config["sourcefile"] == config["outfile"]
            and config["outfile"] not in [STDOUT, MODULEOUT]
            and not gui
        ):
            Error(
                _("Input and Output files are the same: %s")
                % config["outfile"]
            )
        return config
    ...
    def parse(self):
        ...
        for target, key, value in raw:
            self.add(key, value)
        Message(
            _("Added the following keys: %s") % ", ".join(self.parsed.keys()),
            2,
        )
...
class TitleMaster:
    ...
    def _set_prop(self, line=""):
        ...
        if rules["titleblocks"]:
            ...
            self.tag = TAGS.get(kind + repr(level)) or TAGS.get(
                "title" + repr(level)
            )
    ...
    def _tag_cells(self, rowdata):
        ...
        for cellindex in range(0, len(rowdata["cellspan"])):
            ...
            if span > 1:
                ...
                mcopen = regex["x"].sub(
                    str(span), TAGS["_tableCellMulticolOpen"]
                )
        ...
        for cell in cells:
            ...
            if rules["tablecellmulticol"]:
                if this_mcopen:
                    copen = regex["_tableColAlign"].sub(
                        this_align, this_mcopen
                    )
...
def toc_formatter(toc, config):
    ...
    if config["target"] == "art" and config["slides"]:
        ...
        toc = (
            aa_slide(_("Table of Contents"), config["width"])
            + toc
            + ([""] * n)
        )
...
def doHeader(headers, config):
    ...
    if target == "art":
        n_h = len(
            [v for v in head_data if v.startswith("HEADER") and head_data[v]]
        )
        if not n_h:
            return []
        if config["slides"]:
...
def get_tagged_link(label, url):
    ...
    guessurl = ""
    if linktype == "url" and re.match(
        "(?i)" + regex["_urlskel"]["guess"], url
    ):
        if url[0] in "Ww":
            guessurl = "http://" + url
        else:
...
def process_source_file(file_="", noconf=0, contents=[]):
    ...
    if not noconf:
        ...
        Message(
            _("Parsing and saving all config found (%03d items)")
            % (len(full_raw)),
            1,
        )
...
def convert_this_files(configs):
    ...
    for myconf, doc in configs:  # multifile support
        ...
        if (
            ...
            and myconf["target"] == "art"
        ):
            n = (myconf["height"] - 1) - (
                AA_COUNT % (myconf["height"] - 1) + 1
            )
            target_body = (
                target_body
                + ([""] * n)
                + [aa_line(AA["bar2"], myconf["width"])]
            )
...
def convert(bodylines, config, firstlinenr=1):
    ...
    while lineref < len(bodylines):
        ...
        if (
            regex["blockRawOpen"].search(line)
            and BLOCK.block() not in BLOCK.exclusive
        ):
        ...
        if (
            regex["blockVerbOpen"].search(line)
            and BLOCK.block() not in BLOCK.exclusive
        ):
        ...
        if regex["special"].search(line):
            ...
            if key == "include":
                ...
                if inctype != "t2t":
                    ...
                    bodylines = (
                        bodylines[:lineref] + inclines + bodylines[lineref:]
                    )
            ...
            # %!csv command
                ...
                if not csv:
                    Error(
                        "Python module 'csv' not found, but needed for %!csv"
                    )
...
class Gui:
    ...
    def action(self, txt):
        self.label(
            txt, fg=self.fg1, bg=self.bg1, wraplength=self.action_length
    ...
    def frame_close(self):
        self.window.grid(
            column=1, row=self.row, sticky="w", padx=self.frame_margin
        )
    ...
    def runprogram(self):
        ...
        if not infile:
            showwarning(
                my_name, _("You must provide the source file location!")
            )
            return
        # Compose cmdline
        guiflags = []
    ...
    def mainwindow(self):
        ...
        # Header
        self.label(
            "%s %s" % (my_name.upper(), my_version), bg=self.bg2, fg=self.fg2
        ...
        if outfile:
            ...
            if outfile == STDOUT:
                txt = _("<screen>")
            l_output = self.label(
                _("Output: ") + txt, fg=self.fg2, bg=self.bg2
            )
        ...
        for setting in ["style", "encoding"]:
            if self.conf.get(setting):
                ...
                self.label(
                    "%s: %s" % (name, val), fg=self.fg2, bg=self.bg2
                ).grid(columnspan=2, sticky="w")
        # Choose target
        self.frame_close()
        self.action(action2)
        self.frame_open()
