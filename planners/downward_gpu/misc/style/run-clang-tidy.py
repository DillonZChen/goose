...
def check_search_code_with_clang_tidy():
    ...
    with open(os.devnull, "w") as devnull:
        subprocess.check_call(
            ["cmake", "../../src"], cwd=build_dir, stdout=devnull
        )
    ...
    compile_commands = [
        {
            ...
            "command": "g++ -I{}/ext -std=c++11 -c {}".format(
                search_dir, src_file
            ),
            "file": src_file,
        }
    ...
    try:
        output = subprocess.check_output(
            cmd, cwd=DIR, stderr=subprocess.STDOUT
        ).decode("utf-8")
        ...
        return False
    errors = re.findall(
        r"^(.*:\d+:\d+: .*(?:warning|error): .*)$", output, flags=re.M
    )
    for error in errors:
        print(error)
    if errors:
        ...
        print()
        print(
            "You may be able to fix these issues with the following command: "
