...
def configs_optimal_extended():
    return {
        "astar_cegar": ["--search", "astar(cegar())"],
...
def configs_optimal_lp(lp_solver="cplex"):
    return {
        "divpot": [
            "--search",
            f"astar(diverse_potentials(lpsolver={lp_solver}))",
        ],
        "seq+lmcut": [
            "--search",
            f"astar(operatorcounting([state_equation_constraints(), lmcut_constraints()], lpsolver={lp_solver}))",
        ],
    }
