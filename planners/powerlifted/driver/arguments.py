...
EVALUATOR_CHOICES = ["blind", "goalcount", "add", "max", "ff", "rff", "gnn"]
...
def parse_options():
    ...
    parser.add_argument(
        ...
        dest="cpu",
        action="store_true",
        help="Run planner in cpu mode.",
    )
    parser.add_argument(
        "--gpu",
        dest="gpu",
        action="store_true",
        help="Run planner in gpu mode.",
    )
    parser.add_argument(
        "--cxx-compiler",
        default="default",
        help="Path to CXX compiler used by CMake.",
    )
    parser.add_argument(
        "-s",
        "--search",
    ...
    parser.add_argument(
        ...
        dest="model_file",
        default="",
        help="Input file of saved GNN model",
    )
    parser.add_argument(
        "-g",
        "--generator",
    ...
    parser.add_argument(
        "--seed", action="store", help="Random seed.", default=1
    )
    parser.add_argument(
        "--time-limit",
        action="store",
    ...
    if args.domain is None:
        ...
        if args.domain is None:
            raise RuntimeError(
                f'Could not find domain filename matching instance file "{args.instance}"'
            )
    ...
    if args.iteration is not None:
        ...
        for it in args.iteration:
            ...
            if not time >= 0 and not time <= 100:
                print(
                    f"relative time {time} not valid (relative time should be in [0,100])"
                )
