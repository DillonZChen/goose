...
class JoinRule(BuildRule):
    ...
    def validate(self):
        ...
        right_vars = {
            v for v in right_args if isinstance(v, int) or v[0] == "?"
        }
...
class ProductRule(BuildRule):
    ...
    def validate(self):
        assert len(self.conditions) >= 2, self
        cond_vars = [
            {v for v in cond.args if isinstance(v, int) or v[0] == "?"}
            for cond in self.conditions
        ]
        all_cond_vars = reduce(set.union, cond_vars)
        eff_vars = {
            v for v in self.effect.args if isinstance(v, int) or v[0] == "?"
        }
...
class Queue:
    def __init__(self, atoms):
        ...
        self.queue_pos = 0
        self.enqueued = {
            (atom.predicate,) + tuple(atom.args) for atom in self.queue
        }
