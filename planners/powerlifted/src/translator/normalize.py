...
class PreconditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules, add_inequalities):
        ...
        rule_body = condition_to_rule_body(
            action.parameters, self.condition, add_inequalities
        )
...
class EffectConditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules, add_inequalities):
        ...
        if not rule_head.negated:
            ...
            rule_body += condition_to_rule_body(
                [], self.condition, add_inequalities
            )
...
class AxiomConditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules, add_inequalities):
        ...
        app_rule_body = condition_to_rule_body(
            axiom.parameters, self.condition, add_inequalities
        )
...
def move_existential_quantifiers(task):
    def recurse(condition):
        ...
        if isinstance(condition, pddl.ExistentialCondition):
            new_parameters = (
                condition.parameters + existential_parts[0].parameters
            )
...
def condition_to_rule_body(parameters, condition, add_inequalities):
    ...
    if not isinstance(condition, pddl.Truth):
        ...
        for part in parts:
            ...
            assert isinstance(part, pddl.Literal), (
                "Condition not normalized: %r" % part
            )
            if not part.negated:
                result.append(part)
            else:
