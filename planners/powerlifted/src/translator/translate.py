...
if not python_version_supported():
    sys.exit(
        "Error: Translator only supports Python >= 2.7 and Python >= 3.2."
    )
...
def print_objects(task, object_index, type_index, types_dict):
    ...
    for index, obj in enumerate(task.objects):
        ...
        print(
            " ".join(
                str(type_index[t]) for t in sorted(types_dict[obj.type_name])
            )
        )
...
def remove_static_predicates_from_goal(task, static_pred):
    ...
    if removed > 0:
        print(
            "Removing satisfied static predicates from the goal.",
            file=native_stdout,
        )
...
def is_trivially_unsolvable(task, static_pred):
    ...
    if isinstance(task.goal, pddl.conditions.Atom) or isinstance(
        task.goal, pddl.Atom
    ):
        if task.goal.predicate in static_pred and violated_in_initial_state(
            ...
            return True
    for g in task.goal.parts:
        if g.predicate in static_pred and violated_in_initial_state(
            task.init, g
        ):
