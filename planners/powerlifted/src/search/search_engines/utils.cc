...
double get_wall_time(){  // cpu time not accurate when using gpu
    struct timeval time;
...
void print_no_solution_found(const double& timer_start) {
    ...
    cout << "Total time: " << get_wall_time() - timer_start << endl;
}
...
{
    cout << "Goal found at: " << get_wall_time() - timer_start << endl;
    cout << "Total time: " << get_wall_time() - timer_start << endl;
}
