...
class SASTask:
    ...
    def output(self, stream):
        ...
        print(len(self.metrics), file=stream)
        for metric in self.metrics:
            assert(type(metric) is PrimitiveNumericExpression)
...
class SASOperator:
    ...
    def validate(self, variables):
        ...
        for cost in self.cost:
            assert cost >= 0 and cost == int(cost)
    ...
    def output(self, stream):
        ...
        print(len(self.cost), file=stream)
        for c in self.cost:
            print(c, end= ' ', file=stream)
        #print(self.cost, file=stream)
        print("\nend_operator", file=stream)
