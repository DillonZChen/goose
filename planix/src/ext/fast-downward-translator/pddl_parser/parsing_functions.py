...
from pddl.f_expression import PrimitiveNumericExpression
...
def parse_effects(alist, result, type_dict, predicate_dict):
    ...
    cost_effs, rest_effect = normalized.extract_cost()
...
def parse_task(domain_pddl, task_pddl):
    ...
    task_name, task_domain_name, task_requirements, objects, init, goal, metrics = parse_task_pddl(task_pddl, type_dict, predicate_dict)
    ...
    return pddl.Task(
        ...
        predicates, functions, init, goal, actions, axioms, metrics)
...
def parse_task_pddl(task_pddl, type_dict, predicate_dict):
    ...
    for entry in iterator:
        if entry[0] == ":metric":
            assert entry[1] == "minimize"
            for metric in entry[2:]:
                # each metric is an n-ary function assignment, assigning the
                # function symbol (the metric) its arguments
                metric = PrimitiveNumericExpression(metric[0], metric[1:])
