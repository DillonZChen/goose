        ...
        if effects:
            if len(metrics) > 0:
                ...
                if self.costs is not None:
                    for cost_eff in self.costs:
                        # Instantiate the cost effect and put it at correct cost
                        # index
                        cost_expr = cost_eff.effect.instantiate(var_mapping, init_facts)
                        metric = cost_expr.fluent
                        cost_index = metrics.index(metric)
                        cost[cost_index] = int(cost_expr.expression.value)
            else:
                cost = 1
