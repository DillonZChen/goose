...
class PrimitiveNumericExpression(FunctionalExpression):
    ...
    def output(self, stream):
        print("%s(%s)" % (self.symbol, ", ".join(map(str, self.args))), file=stream)
    def instantiate(self, var_mapping, init_facts):
...
class FunctionAssignment(object):
    ...
    def instantiate(self, var_mapping, init_facts):
        ...
        # assignments, "instantiate" is not called) and therefore a constant (we
        # do not allow for conditional costs). Hence, we know that the fluent is
        # a metric which is instantiated here
        args = [var_mapping.get(arg, arg) for arg in self.fluent.args]
        fluent = PrimitiveNumericExpression(self.fluent.symbol, args)
