...
def translate_task(strips_to_sas, ranges, translation_key,
                   ...
                   init, goals,
                   actions, axioms, metrics, implied_facts):
...
def solvable_sas_task(msg):
    if not options.quite: print("%s! Generating solvable task..." % msg)
    return trivial_task(solvable=True)

def unsolvable_sas_task(msg):
    if not options.quite: print("%s! Generating unsolvable task..." % msg)
...
def pddl_to_sas(task):
    ...
    with timers.timing("Translating task", block=True):
        sas_task = translate_task(
            ...
            mutex_dict, mutex_ranges, mutex_key,
            task.init, goal_list, actions, axioms, task.metrics,
            implied_facts)
    ...
    for group in groups:
        ...
        for fact in group:
            if strips_to_sas.get(fact):
                ...
                if not options.quite: print("not in strips_to_sas, left out:", fact)
...
def main():
    timer = timers.Timer()
    timers.QUITE = options.quite
    build_model.QUITE = options.quite
    ...
    if not options.quite: print("Done! %s" % timer)
