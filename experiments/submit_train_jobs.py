...
def main():
    ...
    for domain in Perc(DOMAINS):
        ...
        DATA_TOML = f"{CUR_DIR}/{domain}.toml"
        with open(DATA_TOML, "w") as f:
            f.write(
                f"domain_pddl = '{PDDL_BENCHMARK_DIR}/{domain}/domain.pddl'\n")
            f.write(f"tasks_dir = '{PDDL_BENCHMARK_DIR}/{domain}/training'\n")
            f.write(
                f"plans_dir = '{PDDL_BENCHMARK_DIR}/{domain}/training_plans'\n")

        # Generate all combinations of configurations
        for config in CONFIG_COMBINATIONS:
            description = "_".join(map(str, [domain] + list(config)))
            ...
            model_file = f"{CONFIGS['MODEL_DIR']}/{description}.model"
            ...
            if args.remove_terminated:
                ...
                with open(log_file) as f:
                    content = f.read()
                if "SIGTERM Termination" in content or "SIGKILL Kill" in content or "CANCELLED AT" in content:
                    os.remove(log_file)
            ...
            if args.remove_failed:
                ...
                with open(log_file) as f:
                    content = f.read()
                if "FATAL:   container creation failed: mount" in content or "Started parsing training data" not in content or "OSError" in content:
                    os.remove(log_file)
                    print(f"Removed {description}")
                continue
            ...
            cmd = f"apptainer run --bind {CUR_DIR}:{CUR_DIR} {GOOSE_SIF} train {DATA_TOML} -s {model_file}"
            for var, val in zip(MODEL_CONFIG_VARS, config):
                cmd += f" --{var.replace('_', '-')} {val}"
            job_vars = ",".join([f"CMD={cmd}", f"TMP_DIR={tmp_dir}"])

            job_cmd = [
                "sbatch",
                f"--job-name=train_{description}",
                f"--output={log_file}",
                f"--export={job_vars}",
                JOB_SCRIPT,
            ]
