...
SearchFactory::create(const Options &opt, const std::string& method, const std::string& state_type) {
    ...
    else if (boost::iequals(method, "gbbfs")) {
        if (using_ext_state) return new GreedyBatchBestFirstSearch<ExtensionalPackedState>();
        else return new GreedyBatchBestFirstSearch<SparsePackedState>();
    }
    ...
    else if (boost::iequals(method, "blazy")) {
        if (using_ext_state) return new BatchLazySearch<ExtensionalPackedState>(true, false);
        else return new BatchLazySearch<SparsePackedState>(true, false);
    }
    ...
    else if (boost::iequals(method, "perfect")) {
        if (using_ext_state) return new PerfectSearch<ExtensionalPackedState>();
        else return new PerfectSearch<SparsePackedState>();
    }
    else {
