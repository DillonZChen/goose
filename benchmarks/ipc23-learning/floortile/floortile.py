...
def get_objects(rows: int, columns: int, robots: int, **kwargs: dict) -> str:
    ...
    str_objects += (
        offset
        + offset.join([f"robot{i+1}" for i in range(robots)])
        + " - robot"
    )
...
def get_init(rows: int, columns: int, robots: int, **kwargs: dict) -> str:
    ...
    for r in range(robots):
        ...
        str_init += (
            offset + f"(robot-at robot{r+1} {get_tile(row,robot_cols[r])})"
        )
    ...
    str_init += offset + offset.join(
        [
            f"(up {get_tile(r+1,c)} {get_tile(r, c)} )"
    ...
    str_init += offset + offset.join(
        [
            f"(down {get_tile(r-1, c)} {get_tile(r, c)} )"
    ...
    str_init += offset + offset.join(
        [
            f"(left {get_tile(r, c-1)} {get_tile(r, c)} )"
            for r in range(0, 1 + rows)
            for c in range(2, 1 + columns)
        ]
    )
    # Right direction
    str_init += offset + offset.join(
        [
            f"(right {get_tile(r, c+1)} {get_tile(r, c)} )"
            for r in range(0, 1 + rows)
...
def get_goal(rows: int, columns: int, **kwargs: dict) -> str:
    ...
    str_goal += offset + offset.join(
        [
            f"(painted {get_tile(r,c)} {colors[(r+c)%2]})"
            for r in range(1, 1 + rows)
            for c in range(1, 1 + columns)
        ]
    )
...
def parse_args() -> Dict[str, int]:
    ...
    parser.add_argument(
        "-r", "--rows", type=int, help="number of rows (min 2)", required=True
    )
    parser.add_argument(
        "-c",
        "--columns",
        type=int,
        help="number of columns (min 2)",
        required=True,
    )
    parser.add_argument(
        "-p",
    ...
    if columns < 2:
        logging.error(
            f" At least 2 columns required (input: columns={columns})\n"
        )
        sys.exit(-2)
    if robots > columns:
        logging.error(
            f" Robots must be less or equal to columns (input: robots={robots}; columns={columns})\n"
        )
        sys.exit(-3)
