...
def get_next_config(
    ...
    step_satellites = (
        float(1 + max_satellites - starting_satellites) / num_instances
    )
    step_instruments = (
        float(1 + max_instruments - starting_instruments) / num_instances
    )
    step_modes = float(1 + max_modes - starting_modes) / num_instances
    step_directions = (
        float(1 + max_directions - starting_directions) / num_instances
    )
    while instance_id <= max_instance_id:
        ...
        yield f"PYTHONHASHSEED=0 python satellite.py -s {satellites} -i {instruments} -m {modes} -d {directions} " f"-o {out_folder} -id {instance_id} --seed {seed}"
