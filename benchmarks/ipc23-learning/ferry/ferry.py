...
def get_objects(cars: int, locations: int, **kwargs: dict) -> str:
    ...
    # -- cars
    str_objects += (
        " ".join([f"car{i}" for i in range(1, 1 + cars)]) + " - car\n    "
    )

    # -- locations
    str_objects += (
        " ".join([f"loc{i+1}" for i in range(locations)]) + " - location\n "
    )
...
def get_init(
    cars: int, locations: int, **kwargs: dict
) -> Tuple[str, List[int]]:
    str_init = "\n"
...
def get_goal(
    cars: int, locations: int, origin_car_locations: List[int], **kwargs: dict
) -> str:
    goal_car_locations = []
    for i in range(cars):
        goal_car_locations.append(
            random.choice(
                [
                    dest
                    for dest in range(1, 1 + locations)
                    if dest != origin_car_locations[i]
                ]
            )
        )
    ...
    return (
        ...
        + "\n   ".join(
            [
                f"(at car{i} loc{goal_car_locations[i-1]})"
                for i in range(1, 1 + cars)
            ]
        )
        + ")"
...
def parse_args() -> Dict[str, int]:
    ...
    parser.add_argument(
        "-c", "--cars", type=int, help="number of cars (min 1)", required=True
    )
    parser.add_argument(
        "-l",
        "--locations",
    ...
    if cars < 1:
        logging.error(
            f"[ERROR] At least 1 car required (input: cars={cars})\n"
        )
        sys.exit(-1)
    if locations < 2:
        logging.error(
            f"[ERROR] At least 2 locations required (input: locations={locations})\n"
        )
        sys.exit(-2)
