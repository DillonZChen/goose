...
def get_next_config(
    ...
    step_waypoints = (
        float(1 + max_waypoints - starting_waypoints) / num_instances
    )
    step_cameras = float(1 + max_cameras - starting_cameras) / num_instances
    step_objectives = (
        float(1 + max_objectives - starting_cameras) / num_instances
    )
    ...
    while instance_id <= max_instance_id:
        ...
        yield f"PYTHONHASHSEED=0 python rovers.py -r {rovers} -w {waypoints} -c {cameras} -o {objectives} " f"-out {out_folder} -id {instance_id} --seed {seed}"
