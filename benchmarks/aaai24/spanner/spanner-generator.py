...
def get_init(args):
    ...
    str_init = (
        str_init + "    (link location" + str(args.locations) + " gate)\n"
    )
...
def parse_args():
    ...
    parser.add_argument(
        "spanners", type=int, help="number of spanners (min 1)"
    )
    parser.add_argument(
        "nuts", type=int, help="number of nuts (min 1 and <= spanners)"
    )
    parser.add_argument(
        "locations", type=int, help="number of locations (min 1)"
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=None,
        help="random seed (default: use time-based seed)",
    )
    parser.add_argument(
        "--problem-name", default="prob", help="number of locations (min 1)"
    )
