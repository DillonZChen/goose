...
def get_objects(
    ...
    **kwargs: dict,
    ...
    # -- satellites
    str_objects += (
        " ".join([f"sat{i}" for i in range(1, 1 + satellites)])
        + " - satellite\n    "
    )

    # -- instruments
    str_objects += (
        " ".join([f"ins{i}" for i in range(1, 1 + instruments)])
        + " - instrument\n    "
    )

    # -- modes
    str_objects += (
        " ".join([f"mod{i}" for i in range(1, 1 + modes)]) + " - mode\n    "
    )

    # -- directions
    str_objects += (
        " ".join([f"dir{i}" for i in range(1, 1 + directions)])
        + " - direction\n    "
    )
...
def get_init(
    ...
    **kwargs: dict,
    ...
    str_init = offset + offset.join(
        [
            f"(pointing sat{s} dir{random.randint(1,directions)})"
            for s in range(1, 1 + satellites)
        ]
    )
    # 2. All satellites have power available
    str_init += offset + offset.join(
        [f"(power_avail sat{s})" for s in range(1, 1 + satellites)]
    )

    # 3. Instruments have random calibration targets
    str_init += offset + offset.join(
        [
            f"(calibration_target ins{i} dir{random.randint(1,directions)})"
    ...
    onboard_instruments = [
        f"(on_board ins{s} sat{satellite_ids[s-1]})"
        for s in range(1, 1 + satellites)
    ]
    # 4.b. the rest of instruments randomly assigned
    onboard_instruments.extend(
        [
            f"(on_board ins{i} sat{random.randint(1, satellites)})"
...
def get_goal(
    satellites: int, directions: int, modes: int, **kwargs: dict
) -> str:
    str_goal = " (and "
    ...
    observation_combs = [
        (d + 1, m + 1) for d in range(directions) for m in range(modes)
    ]
...
def parse_args() -> Dict[str, int]:
    ...
    parser.add_argument(
        ...
        "--satellites",
        type=int,
        help="number of satellites (min 1)",
        required=True,
    )
    parser.add_argument(
        "-i",
        "--instruments",
        type=int,
        help="number of instruments (min 1)",
        required=True,
    )
    parser.add_argument(
        "-m",
    ...
    parser.add_argument(
        ...
        "--directions",
        type=int,
        help="number of directions (min 2)",
        required=True,
    )
    parser.add_argument(
    ...
    if satellites < 1:
        logging.error(
            f" At least 1 satellite required (input: satellites={satellites})\n"
        )
        sys.exit(-1)
    if instruments < 1:
        logging.error(
            f" At least 1 instrument required (input: instruments={instruments})\n"
        )
        sys.exit(-2)
    if modes < 1:
    ...
    if directions < 2:
        logging.error(
            f" At least 2 directions required (input: directions={directions})\n"
        )
        sys.exit(-4)
