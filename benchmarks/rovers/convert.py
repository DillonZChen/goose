    ...
    for c_dir, n_dir in [
        ...
        for f_name in tqdm(sorted(os.listdir(c_dir))):
            ...
            for obj in problem.all_objects:
                if obj._typename == user_types["waypoint"]:
                    waypoints.append(obj)
                elif obj._typename == user_types["rover"]:
                    rovers.append(obj)

            problem.set_initial_value(fluents["recharges"], 0)

            energy = fluents["energy"]
            in_sun = fluents["in_sun"]

            for rover in rovers:
                problem.set_initial_value(energy(rover), 16)
            for i, waypoint in enumerate(waypoints):
                if i % 3 != 0:  # 2/3 of waypoints are in sun
                    problem.set_initial_value(in_sun(waypoint), True)
