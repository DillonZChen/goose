...
def get_objects(
    rovers: int, waypoints: int, cameras: int, objectives: int, **kwargs: dict
) -> str:
    offset = "\n    "
    ...
    str_objects += (
        offset
        + " ".join([get_rover(i) for i in range(1, 1 + rovers)])
        + " - rover"
    )
    str_objects += (
        offset
        + " ".join([get_store(i) for i in range(1, 1 + rovers)])
        + " - store"
    )
    ...
    str_objects += (
        offset
        + " ".join([get_waypoint(i) for i in range(1, 1 + waypoints)])
        + " - waypoint"
    )
    ...
    str_objects += (
        offset
        + " ".join([get_camera(i) for i in range(1, 1 + cameras)])
        + " - camera"
    )
    ...
    str_objects += (
        offset
        + " ".join([get_objective(i) for i in range(1, 1 + objectives)])
        + " - objective"
    )
...
def get_init(
    ...
    str_init += (
        offset
        + f"(at_lander {get_lander()} {get_waypoint(random.randint(1,waypoints))})"
    )
    ...
    str_init += offset + offset.join(
        [
            f"(at {r} {get_waypoint(random.randint(1,waypoints))})"
            for r in all_rovers
        ]
    )
    ...
    str_init += offset + offset.join(
        [
            f"(equipped_for_soil_analysis {r})"
    ...
    str_init += offset + offset.join(
        [
            f"(equipped_for_rock_analysis {r})"
            for r in all_rovers[0 : random.randint(1, rovers)]
        ]
    )
    random.shuffle(all_rovers)
    ...
    str_init += offset + offset.join(
        [f"(equipped_for_imaging {r})" for r in imaging_rovers]
    )

    # One empty store per rover
    str_init += offset + offset.join(
        [f"(empty {get_store(r)})" for r in range(1, 1 + rovers)]
    )
    str_init += offset + offset.join(
        [
            f"(store_of {get_store(r)} {get_rover(r)})"
    ...
    if rock_samples:
        str_init += offset + offset.join(
            [f"(at_rock_sample {r})" for r in rock_samples]
        )
    if soil_samples:
        str_init += offset + offset.join(
            [f"(at_soil_sample {s})" for s in soil_samples]
        )
    ...
    str_init += offset + offset.join(
        [
            f"(visible {get_waypoint(a)} {get_waypoint(b)})"
    ...
    for r in range(1, 1 + rovers):
        ...
        str_init += offset + offset.join(
            [
                f"(can_traverse {get_rover(r)} {get_waypoint(a)} {get_waypoint(b)})"
                for a, b in graph_r
            ]
        )
    ...
    for c in range(1, 1 + cameras):
        ...
        str_init += (
            offset
            + f"(calibration_target {get_camera(c)} {get_objective(r_obj)})"
        )
        ...
        str_init += offset + offset.join(
            [f"(supports {get_camera(c)} {m})" for m in modes[0:num_modes]]
        )
    ...
    for m in get_modes():
        ...
        str_init += offset + offset.join(
            [f"(supports {c} {m})" for c in all_cameras[0:num_cam]]
        )
    ...
    for o in range(1, 1 + objectives):
        ...
        str_init += offset + offset.join(
            [
                f"(visible_from {get_objective(o)} {w})"
...
def get_goal(
    ...
    **kwargs: dict,
    ...
    str_goal += offset + offset.join(
        [f"(communicated_rock_data {r})" for r in rock_samples[0:n_rock_goals]]
    )
    ...
    str_goal += offset + offset.join(
        [f"(communicated_soil_data {r})" for r in soil_samples[0:n_soil_goals]]
    )

    # Random image goals
    all_objectives_modes = [
        (get_objective(o), m)
        for o in range(1, 1 + objectives)
        for m in get_modes()
    ]
    ...
    str_goal += offset + offset.join(
        [
            f"(communicated_image_data {obj} {mod})"
...
def parse_args() -> Dict[str, int]:
    ...
    parser.add_argument(
        ...
        "--waypoints",
        type=int,
        help="number of waypoints (min 2)",
        required=True,
    )
    parser.add_argument(
        "-c",
    ...
    parser.add_argument(
        ...
        "--objectives",
        type=int,
        help="number of objectives (min 1)",
        required=True,
    )
    parser.add_argument(
    ...
    if waypoints < 2:
        logging.error(
            f" At least 2 waypoints required (input: waypoints={waypoints})\n"
        )
        sys.exit(-2)
    if cameras < 1:
        logging.error(
            f" At least 1 camera required (input: cameras={cameras})\n"
        )
        sys.exit(-3)
    if objectives < 1:
        logging.error(
            f" At least 1 objective required (input: objectives={objectives})\n"
        )
        sys.exit(-4)
