...
def generalize_plan(pddl_problem) -> SequentialPlan:
    ...
    g_at = {str(g.args[0]): pddl_problem.object(str(g.args[1])) for g in goal_fluents if str(g).split('(')[0] == "at"}
    ...
    with SequentialSimulator(problem=pddl_problem) as simulator:
        ...
        for k,v in state._values.items():
            ...
            if fluent[0] == "at":  # get where the vehicle/package is at
                locatable_at[str(k.args[0])] = pddl_problem.object(str(k.args[1]))
            elif fluent[0] == "road":  # build the underlying graph
                loc1, loc2 = pddl_problem.object(str(k.args[0])), pddl_problem.object(str(k.args[1]))
                if loc1 in graph.keys():
                    graph[loc1].append(loc2)
                else:
                    graph[loc1] = [loc2]
            elif fluent[0] == "capacity":  # get the car capacity
                capacities[str(k.args[0])] = pddl_problem.object(str(k.args[1]))
            elif fluent[0] == "capacity-predecessor":  # get the capacity predecessor
                capacity_predecessor[str(k.args[1])] = pddl_problem.object(str(k.args[0]))
        ...
        def get_path(from_loc, to_loc):
            ...
            assert (visited_from[to_loc] != None)  # the graph is strongly connected, so there must be always a path
            path = []
            current_loc = to_loc
        ...
        for package_str, goal_package_loc in g_at.items():
            ...
            for next_loc in path:
                drive = ActionInstance(actions[0], tuple([vehicle, vehicle_at, next_loc]))
            ...
            pick_up = ActionInstance(actions[1], tuple([vehicle, vehicle_at, package_obj, v_cap_pre, v_cap]))
            ...
            # 4. drop the package
            drop = ActionInstance(actions[2], tuple([vehicle, vehicle_at, package_obj, v_cap_pre, v_cap]))
...
def main():
    ...
    """
    all_problems = [f"../../transport/training/easy/p{p:02}.pddl" for p in range(1, 100)]
    all_problems.extend([f"../../transport/testing/easy/p{p:02}.pddl" for p in range(1, 31)])
    all_problems.extend([f"../../transport/testing/medium/p{p:02}.pddl" for p in range(1, 31)])
    all_problems.extend([f"../../transport/testing/hard/p{p:02}.pddl" for p in range(1, 31)])
    ...
    for prob, plan_file in zip(all_problems, all_plans):
        ...
        pddl_problem = reader.parse_problem('../../transport/domain.pddl', prob)
        plan = generalize_plan(pddl_problem)
