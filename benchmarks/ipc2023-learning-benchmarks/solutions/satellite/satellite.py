...
def generalize_plan(pddl_problem) -> SequentialPlan:
    ...
    actions = pddl_problem.actions  # 0: turn_to, 1: switch_on, 2: switch_off, 3: calibrate, 4: take_image
    # print(actions)
    ...
    g_sat = {str(g.args[0]):pddl_problem.object(str(g.args[1])) for g in goal_fluents if str(g).split('(')[0] == "pointing"}
    ...
    with SequentialSimulator(problem=pddl_problem) as simulator:
        ...
        for k,v in state._values.items():
            ...
            if fluent[0] == "pointing":  # get where the satellite is at
                satellites_at[str(k.args[0])] = pddl_problem.object(str(k.args[1]))
            elif fluent[0] == "calibration_target":  # get the target of each instrument
                calibration_targets[str(k.args[0])] = pddl_problem.object(str(k.args[1]))
        ...
        for s in sat_objs:
            ...
            for i in ins_objs:
                ...
                turn_to = ActionInstance(actions[0], tuple([s, dest_dir, current_dir]))
                ...
                calibrate = ActionInstance(actions[3], tuple([s, i, current_dir]))
                ...
                for dir in dir_objs:
                    turn_to = ActionInstance(actions[0], tuple([s, dir, current_dir]))
                    if simulator.is_applicable(state, turn_to):
                        state = simulator.apply(state, turn_to)
                        current_dir = dir
                        plan.append(turn_to)
                    for m in mod_objs:
                        take_image = ActionInstance(actions[4], tuple([s, current_dir, i, m]))
            ...
            if str(s) in g_sat.keys():
                turn_to = ActionInstance(actions[0], tuple([s, g_sat[str(s)], current_dir]))
                if simulator.is_applicable(state, turn_to):
                    state = simulator.apply(state, turn_to)
                    plan.append(turn_to)
...
def main():
    ...
    """
    all_problems = [f"../../satellite/training/easy/p{p:02}.pddl" for p in range(1, 100)]
    all_problems.extend([f"../../satellite/testing/easy/p{p:02}.pddl" for p in range(1, 31)])
    all_problems.extend([f"../../satellite/testing/medium/p{p:02}.pddl" for p in range(1, 31)])
    all_problems.extend([f"../../satellite/testing/hard/p{p:02}.pddl" for p in range(1, 31)])
    ...
    for prob, plan_file in zip(all_problems, all_plans):
        ...
        pddl_problem = reader.parse_problem('../../satellite/domain.pddl', prob)
        plan = generalize_plan(pddl_problem)
