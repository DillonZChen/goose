    ...
    with SequentialSimulator(problem=pddl_problem) as simulator:
        ...
        for r in sorted_robots:
            ...
            while current_row < max_rows:
                next_tile = pddl_problem.object(f"tile_{current_row+1}_{current_col}")
                move = ActionInstance(actions[3], tuple([r, tile, next_tile]))  # move up
            ...
            while prev_col+1 < current_col:
                next_tile = pddl_problem.object(f"tile_{current_row}_{current_col-1}")
                move = ActionInstance(actions[6], tuple([r, tile, next_tile]))  # move left
                assert simulator.is_applicable(state, move)
                state = simulator.apply(state, move)
                plan.append(move)
                tile = next_tile
        ...
        for r in sorted_robots:
            ...
            if ((row+col)%2) == 0:  # odd
                change_white = ActionInstance(actions[0], tuple([r, black_obj, white_obj]))
                ...
                change_black = ActionInstance(actions[0], tuple([r, white_obj, black_obj]))
        ...
        def move_down_and_paint_up(state, idx, robot):
            ...
            while row > 0:
                ...
                move_down = ActionInstance(actions[4], tuple([robot, tile, next_tile]))  # move down
                assert simulator.is_applicable(state, move_down)
                state = simulator.apply(state, move_down)
                local_plan.append(move_down)

                paint_up = ActionInstance(actions[1], tuple([robot, tile, next_tile, robot_color[idx]]))  # paint up
                # print(paint_up)
                # print(state)
                ...
                if robot_color[idx] == white_obj:
                    change_black = ActionInstance(actions[0], tuple([robot, white_obj, black_obj]))
                    assert simulator.is_applicable(state, change_black)
                    state = simulator.apply(state, change_black)
                    local_plan.append(change_black)
                    robot_color[idx] = black_obj
                else:
                    change_white = ActionInstance(actions[0], tuple([robot, black_obj, white_obj]))
        ...
        while col < max_cols:
            ...
            while row < max_rows:
                next_tile = pddl_problem.object(f"tile_{row+1}_{col}")
                move_up = ActionInstance(actions[3], tuple([robot, tile, next_tile]))
            ...
            state, tile, local_plan = move_down_and_paint_up(state, len(sorted_robots)-1, robot)
            row, col = get_row_col(str(tile))
            robots_at[robot] = tile
            plan.extend(local_plan)
