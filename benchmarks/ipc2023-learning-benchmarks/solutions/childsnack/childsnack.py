...
def generalize_plan(pddl_problem) -> SequentialPlan:
    ...
    actions = pddl_problem.actions  # make_sandwich_no_gluten, make_sandwich, put_on_tray, serve_sandwich_no_gluten, serve_sandwich, move_tray
    # print(actions)
    ...
    with SequentialSimulator(problem=pddl_problem) as simulator:
        ...
        places = [o for o in pddl_problem.objects(utypes[5])]  # kitchen, table1 - 2 - 3 (up to 3 tables)
        ...
        gluten_bread = [bread for bread in bread_objs if not (bread in gluten_free_bread)] + \
                       gluten_free_bread[num_gluten_free_sandwiches:]
        gluten_content = [content for content in content_objs if not (content in gluten_free_content)] + \
        ...
        for idx in range(0, num_gluten_free_sandwiches):
            make_sand_no_gluten = ActionInstance(actions[0], tuple([gluten_free_sandwiches[idx],
        ...
        num_gluten_sandwiches = min(len(gluten_sandwiches), len(gluten_bread), len(gluten_content))
        for idx in range(0, num_gluten_sandwiches):
            make_sand = ActionInstance(actions[1], tuple([gluten_sandwiches[idx], gluten_bread[idx], gluten_content[idx]]))
        ...
        for s in made_sandwiches:
            put_on_tray = ActionInstance(actions[2], tuple([s, trays[0]]))  # use always the first tray
        ...
        for idx in range(1, len(places)):  # first place is always the kitchen
            next_at = places[idx]
            move = ActionInstance(actions[5], tuple([trays[0], tray_at, next_at]))
            ...
            for child in child_objs:
                if child in served:
                    continue
                if current_gluten_sand < len(gluten_sandwiches):  # a gluten sand can be served
                    serve_gluten_sand = ActionInstance(actions[4], tuple([
                        gluten_sandwiches[current_gluten_sand], child, trays[0], tray_at]))
                    if simulator.is_applicable(state, serve_gluten_sand):
                        ...
                        continue
                if current_gluten_free_sand < len(gluten_free_sandwiches):  # a gluten free sand can be served
                    serve_gluten_free_sand = ActionInstance(actions[3], tuple([
                        gluten_free_sandwiches[current_gluten_free_sand], child, trays[0], tray_at]))  # gluten-free
...
def main():
    ...
    """
    all_problems = [f"../../childsnack/training/easy/p{p:02}.pddl" for p in range(1, 100)]
    all_problems.extend([f"../../childsnack/testing/easy/p{p:02}.pddl" for p in range(1, 31)])
    all_problems.extend([f"../../childsnack/testing/medium/p{p:02}.pddl" for p in range(1, 31)])
    all_problems.extend([f"../../childsnack/testing/hard/p{p:02}.pddl" for p in range(1, 31)])
    ...
    for prob, plan_file in zip(all_problems, all_plans):
        ...
        pddl_problem = reader.parse_problem('../../childsnack/domain.pddl', prob)
        plan = generalize_plan(pddl_problem)
