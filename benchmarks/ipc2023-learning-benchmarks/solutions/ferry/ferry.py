...
def generalize_plan(pddl_problem) -> SequentialPlan:
    ...
    g_car = {str(g.args[0]):g.args[1] for g in goal_fluents}  # all are at(car,loc) fluents in the goal
    ...
    with SequentialSimulator(problem=pddl_problem) as simulator:
        ...
        for k,v in state._values.items():
            ...
            elif (v.bool_constant_value()) and str(k)[0:3] == "at-":  # get the first ferry location
                at_ferry = k.args[0]
...
def main():
    ...
    """
    all_problems = [f"../../ferry/training/easy/p{p:02}.pddl" for p in range(1, 100)]
    all_problems.extend([f"../../ferry/testing/easy/p{p:02}.pddl" for p in range(1, 31)])
    all_problems.extend([f"../../ferry/testing/medium/p{p:02}.pddl" for p in range(1, 31)])
    all_problems.extend([f"../../ferry/testing/hard/p{p:02}.pddl" for p in range(1, 31)])
