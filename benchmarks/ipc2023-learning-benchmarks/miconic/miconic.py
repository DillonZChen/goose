...
def get_objects(passengers: int, floors: int, **kwargs: dict) -> str:
    ...
    # -- passengers
    str_objects += " ".join([f"p{i+1}" for i in range(passengers)]) + " - passenger\n    "

    # -- floors
    str_objects += " ".join([f"f{i+1}" for i in range(floors)]) + " - floor\n    "
...
def get_init(passengers: int, floors: int, **kwargs: dict) -> str:
    ...
    for i in range(passengers):
        ...
        destin = random.choice([f for f in range(1, floors+1) if f != origin])  # dest != origin for each passenger
...
def parse_args() -> Dict[str, int]:
    ...
    parser.add_argument("-p", "--passengers", type=int, help="number of passengers (min 1)", required=True)
    parser.add_argument("-f", "--floors", type=int, help="number of floors (min 2)", required=True)
    ...
    # Input sanity checks
    if passengers < 1:
        logging.error(f"[ERROR] At least 1 passenger required (input: passengers={passengers})\n")
        sys.exit(-1)
    if floors < 2:
        logging.error(f"[ERROR] At least 2 floors required (input: floors={floors})\n")
        sys.exit(-2)
    ...
    return {'passengers': passengers, 'floors': floors, 'out_folder': out_f, 'instance_id': ins_id, 'seed': args.seed}
