...
def main():
    parser = argparse.ArgumentParser(description='Executing the runner.py over all instances in a dir')
    ...
    instance_names = next(os.walk(instances_dir), (None, None, []))[2]  # [] if no file
    if instance_names:
        instance_names = [f"{instances_dir}{ins_name}" for ins_name in sorted(instance_names) if ins_name[-5:] == ".pddl"]
    else:
