...
def get_objects(spanners: int, nuts: int, locations: int, **kwargs: dict) -> str:
    str_objects = "\n"
    ...
    # -- spanner
    str_objects += " ".join([f"spanner{i+1}" for i in range(spanners)]) + " - spanner\n    "

    # -- nuts
    str_objects += " ".join([f"nut{i+1}" for i in range(nuts)]) + " - nut\n    "

    # -- locations
    str_objects += "shed " + " ".join([f"location{i+1}" for i in range(locations)]) + " gate - location\n "
...
def get_init(spanners: int, nuts: int, locations: int, **kwargs: dict) -> str:
    ...
    for i in range(spanners):
        str_init += f"    (at spanner{i+1} location{random.randint(1, locations)})\n"
...
def parse_args() -> Dict[str, int]:
    ...
    # Input sanity checks
    if spanners < 1:
        logging.error(f"[ERROR] At least 1 spanner required (input: spanners={spanners})\n")
        sys.exit(-1)
    if nuts < 1:
        logging.error(f"[ERROR] At least 1 nut required (input: nuts={nuts})\n")
        sys.exit(-2)
    if locations < 1:
        logging.error(f"[ERROR] At least 1 location required (input: locations={locations})\n")
        sys.exit(-3)
    if nuts > spanners:
        logging.error(f"[ERROR] Nuts must be between [1,spanners] (input: nuts={nuts}; spanners={spanners})\n")
        sys.exit(-4)
